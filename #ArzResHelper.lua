...
function drawSettingsTab()
    ...
    if settingsExpandedTelegram then
        ...
        if ToggleSwitch(u8("Включить TG уведомления"), imgui.ImBool(tgConfig.enabled)) then
        ...
        if ToggleSwitch(u8("Резервный сервер (для России)"), imgui.ImBool(tgConfig.useReserveServer)) then
        ...
        if ToggleSwitch(u8("Уведомления об аренде (Item Market)"), imgui.ImBool(tgConfig.itemMarketEnabled)) then
...
if ToggleSwitch(u8("Ежедневный отчёт"), imgui.ImBool(tgConfig.dailyReportEnabled)) then
...
if ToggleSwitch(u8("Еженедельный отчёт"), imgui.ImBool(tgConfig.weeklyReportEnabled)) then

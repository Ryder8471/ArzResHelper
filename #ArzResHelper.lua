    ...
    downloadUrlToFile(
        ...
        function(id, status)
            ...
            if status == dlstatus.STATUS_ENDDOWNLOADDATA or doesFileExist(zipPath) then
                ...
                local psCmd = 'powershell -WindowStyle Hidden -NoProfile -command "Expand-Archive -Path \'' .. zipPath .. '\' -DestinationPath \'' .. dirml .. '\' -Force"'
                os.execute(psCmd)
            ...
            elseif status == dlstatus.STATUSEX_ENDDOWNLOAD and not doesFileExist(zipPath) then
                installed = true
                sampAddChatMessage(SCRIPT_PREFIX .. "Ошибка загрузки ресурсов.", SCRIPT_COLOR)
            end
        end
    )
end

...
if azVip and not text:find("донат") and not extractChatSenderNick(text) then
            local now = os.time()
            for i = #pendingPaydayAZ, 1, -1 do
                if now - pendingPaydayAZ[i].time > 60 then
                    table.remove(pendingPaydayAZ, i)
    ...
    if invSectionTab.v == 0 then
        ...
        if not invStatsCache or invStatsCache.len ~= #investmentLog or invStatsCache.date ~= todayDate or invStatsCache.wday ~= mskWday then
            ...
            invStatsCache = {len = #investmentLog, date = todayDate, wday = mskWday,
                              allDates = allDates, seenDates = seenDates,
    ...
    if invSectionTab.v == 1 then
        if next(investmentConfig) == nil then
            ...
            for itemId, data in pairs(investmentConfig) do
                ...
                drawList:AddText(imgui.ImVec2(textX, cardY2 + 13), 0xFFFFFFFF, u8(data.name))
                local countStr = u8("Кол-во: ") .. data.count
                drawList:AddText(imgui.ImVec2(textX, cardY2 + 35), 0xFF999999, countStr)
                drawList:AddText(imgui.ImVec2(textX + imgui.CalcTextSize(countStr).x + 14, cardY2 + 35), caseAccent, formatNumber(data.cost * data.count) .. "$")
                ...
                if imgui.InputInt("##inv_cost_" .. itemId, invEditCache[itemId], 0, 0) then
                ...
                imgui.SetCursorScreenPos(imgui.ImVec2(resetX, resetY))
                if imgui.InvisibleButton("##reset_inv_" .. itemId, imgui.ImVec2(resetSz, resetSz)) then
            ...
            if selectedDate then
                for i = 1, math.min(#cryptoLog, 500) do
                    ...
                    if getGameDate(log.time) == selectedDate then
                        local cost = ((cryptoConfig[log.itemId] and cryptoConfig[log.itemId].cost) or 0) * (log.amount or 1)
                        daySum = daySum + cost
                        logCount = logCount + (log.amount or 1)
                    end
                end
            end
            local btnW = listW; local btnH = 38
...
function drawAboutTab()
    ...
    local devServerIcon = serverIcons[devServerName]
...
function drawMainMenu()
    ...
    if currentTab == 9 or currentTab == 14 or currentTab == 15 or currentTab == 16 or currentTab == 17 or currentTab == 18 or currentTab == 19 or currentTab == 20 then
        local backTarget = (currentTab == 9 or currentTab == 14 or currentTab == 15) and 13 or 5
...
function drawMenuButtons(winPos, winSize, leftPanelWidth, theme)
    ...
    for idx, item in ipairs(menuItems) do
        if item.header then
            ...
            if item.id == currentTab or (item.id == 13 and currentTab == 14) then targetHighlightY = btnPosY end
...
function drawRightPanelContent(winPos, leftPanelWidth, winSize, theme)
    ...
    elseif currentTab == 20 then drawCryptoTab() end
...
function drawProfileCard()
    ...
    if serverIcons[curServerName] then
        drawList:AddImage(serverIcons[curServerName], imgui.ImVec2(cX + 64, cY + 34), imgui.ImVec2(cX + 78, cY + 48))
...
function imgui.OnDrawFrame()
    if not mainWin.v and not isWindowAnimating() and not settings.farmOverlayEnabled and not settings.mineOverlayEnabled and not settings.sawmillOverlayEnabled and not settings.oilOverlayEnabled and #achievementNotifications == 0 and #devNotifications == 0 and #devImportantQueue == 0 and not devImportantCurrent and not notifHistoryPopupOpen then return end

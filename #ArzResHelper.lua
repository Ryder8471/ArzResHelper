...
local select_menu = {true, false, false, false, false, false, false, false, false, false}
...
local function getGameDate(timestamp)
    ...
    if timestamp then
        t = getMoscowTime(timestamp)
    else
        t = getMoscowTime()
    end

    if tonumber(os.date("%H", t)) < 5 then
        t = t - 86400
    end
...
function loadItemMarketStats()
    ...
    -- Парсим: {"time":ЧИСЛО,"nick":"ИМЯ","amount":ЧИСЛО}
...
function getItemMarketWeekIncome()
    if cachedIMWeekTime and os.time() - cachedIMWeekTime < 30 then
    ...
    local currentDay = tonumber(os.date("%w", mskTime))
    if currentDay == 0 then currentDay = 7 end
    if mskHour < 5 then currentDay = currentDay - 1; if currentDay == 0 then currentDay = 7 end end
    for i = 0, currentDay - 1 do
        local date = getGameDate(os.time() - i * 86400)
        for _, log in ipairs(itemMarketLog) do
...
function hook.onServerMessage(color, text)
    ...
    -- Item Market (пассивный, работает всегда)
...
function imgui.BindCard(key, val, winW, theme)
    ...
    drawList:AddText(imgui.ImVec2(delX + 7, delY + 5), 0xFFFFFFFF, fa.ICON_TRASH)
...
function drawSettingsTab()
    ...
    if settingsExpandedNotify then
        imgui.Spacing()
        if imgui.Checkbox(u8("Уведомления о целях в чат"), cb_chatNotify) then settings.chatNotifyEnabled = cb_chatNotify.v; saveConfig(); needSave = true end
        imgui.Spacing()
        if imgui.Checkbox(u8("Звук при выполнении цели"), cb_goalSound) then settings.goalSoundEnabled = cb_goalSound.v; saveConfig(); needSave = true end
        ...
        imgui.Spacing()
        if imgui.Checkbox(u8("Звуки при добыче ресурсов"), cb_pickupSound) then settings.pickupSoundEnabled = cb_pickupSound.v; saveConfig(); needSave = true end
    ...
    if settingsExpandedOverlay then
        imgui.Spacing()
        if imgui.Checkbox(u8("Оверлей фермы"), cb_farm_overlay) then settings.farmOverlayEnabled = cb_farm_overlay.v; saveConfig(); needSave = true end
        if imgui.Checkbox(u8("Оверлей шахты"), cb_mine_overlay) then settings.mineOverlayEnabled = cb_mine_overlay.v; saveConfig(); needSave = true end
        if imgui.Checkbox(u8("Оверлей лесопилки"), cb_sawmill_overlay) then settings.sawmillOverlayEnabled = cb_sawmill_overlay.v; saveConfig(); needSave = true end
        imgui.Spacing()
    end
    ...
    if settingsExpandedTimer then
        imgui.Spacing()
        if imgui.Checkbox(u8("Таймер в оверлее"), cb_overlay_timer) then settings.overlayTimerEnabled = cb_overlay_timer.v; if not cb_overlay_timer.v then overlayTimer.running = false; overlayTimer.elapsed = 0; overlayTimer.displayedTime = "00:00:00" end; saveConfig(); needSave = true end
...
function imgui.OnDrawFrame()
    ...
    imgui.TextColored(hexToImVec4(theme.accent), fa.ICON_WRENCH .. "  Resource Helper")
        ...
        local menuItems = {
            ...
            {title = u8("Цели"), icon = fa.ICON_BULLSEYE, id = 6},
            {title = u8("Достижения"), icon = fa.ICON_STAR, id = 7},
            {title = u8("Биндер"), icon = fa.ICON_KEYBOARD_O, id = 8},
            {title = u8("Настройки"), icon = fa.ICON_WRENCH, id = 9},
            {title = u8("О скрипте"), icon = fa.ICON_SEARCH, id = 10},
        }
    ...
    if not scanState.active then startInventoryScan() end
            ...
            if imgui.InvisibleButton("##prices_farm", imgui.ImVec2(listW, cardH)) then pricesExpandedFarm = not pricesExpandedFarm end
            if pricesExpandedFarm then
                ...
                if StyleButton(u8("Сохранить цены"), fa.ICON_FLOPPY_O) then
    ...
    savePrices(); sampAddChatMessage(SCRIPT_PREFIX.."Цены обновлены!", SCRIPT_COLOR)
        ...
        elseif select_menu[3] then
            ...
            if imgui.InvisibleButton("##prices_mine", imgui.ImVec2(listW, cardH)) then pricesExpandedMine = not pricesExpandedMine end
    ...
    savePrices(); sampAddChatMessage(SCRIPT_PREFIX.."Цены обновлены!", SCRIPT_COLOR)
        ...
        elseif select_menu[4] then
            ...
            if imgui.InvisibleButton("##prices_saw", imgui.ImVec2(listW, cardH)) then pricesExpandedSaw = not pricesExpandedSaw end

...
local settings = {
    ...
    overlayTimerEnabled = false, mineSpawnTimerEnabled = false, overlayStyle = 1, overlayColumns = 2, totalIncomeGoal = 1000000
}
...
function saveStats()
    if not config then return end
    local file = io.open(getServerStatsPath(currentWork), "w")
    if not file then return end
    file:write("{\n  \"logs\": [\n")
...
function loadStats()
    ...
    local file = io.open(getServerStatsPath(currentWork), "r")
...
function sendToLeaderboard(period, mode)
    ...
    if mode == "Income" then
        ...
        for workType, wc in pairs(configs) do
    ...
    local data = getResourcesForPeriod(getServerStatsPath(workType), period)
        ...
        -- Добавляем доход с Item Market и осколков
        if period == "Daily" then
        ...
        elseif period == "Total" then
            for _, log in ipairs(itemMarketLog) do amount = amount + log.amount end
            for _, log in ipairs(shardLog) do amount = amount + log.amount end
        end
...
function loadLeaderboard(period, mode, silent)
    ...
    local tempFile = dirml .. "/lb_cache_" .. mode .. "_" .. period .. ".json"
...
local function parsePaydayLine(text)
    ...
    -- Семейные выплаты -> ТЕПЕРЬ В АКСЫ
    if text:find("Семейные выплаты") then
        local f = text:match("Семейные выплаты[^%d]*([%d%.]+)")
        if f then
            f = f:gsub("%D", "")
            if #f > 0 then 
                paydayAccBonus = paydayAccBonus + tonumber(f)  -- ИЗМЕНЕНО: было paydaySalary
...
function hook.onServerMessage(color, text)
    ...
    -- Логирование ВСЕХ строк пейдея
    if text:find("БАНКОВСКИЙ ЧЕК") or text:find("Вы получили") or text:find("сумма в банке") or text:find("донат") or text:find("Семейные") or text:find("Дивидентный") or text:find("от хар") or text:find("============================================") then
        local f = io.open(dirml .. "/payday_real.log", "a")
        if f then
            f:write(os.date("!%H:%M:%S", os.time()+10800) .. " | " .. text .. "\n")
            f:close()
        end
    end
    
    -- AZ из VIP/рассрочки (приходит ДО чека)
    local azVip = text:match("Вы получили (%d+) AZ")
    if azVip and not text:find("донат") then
        paydayAZ = paydayAZ + tonumber(azVip)
        return
    end
    
    -- ============================================
    -- АКСЕССУАР - СОХРАНЯЕМ В ОТДЕЛЬНУЮ ПЕРЕМЕННУЮ
    -- ============================================
    if text:find("Космическое") or text:find("50.000") or text:find("аксессуара Космическое") then
        paydayAccBonus = 50000  -- Сохраняем бонус отдельно
		paydayAccBreakdown.accessory = 50000
        return
    end
    ...
    if text:find("Дивидентный договор") then
        ...
        if a then
            a = a:gsub("%D", "")
            if #a > 0 then 
                paydayAccBonus = paydayAccBonus + tonumber(a)  -- ИЗМЕНЕНО: было paydaySalary
				paydayAccBreakdown.dividend = tonumber(a)
            end
        end
        
        -- НЕ ПРИБАВЛЯЕМ paydayAccBonus к зарплате, он остаётся отдельно
        
        if paydayActive then
            paydayActive = false
            local totalAmount = paydaySalary + paydayDeposit + paydayAccBonus  -- ИЗМЕНЕНО: добавлен paydayAccBonus
            if totalAmount > 0 or paydayAZ > 0 then
                table.insert(paydayLog, 1, {
                    time = os.time(),
                    salary = paydaySalary,
                    deposit = paydayDeposit,
                    accesories = paydayAccBonus,  -- СОХРАНЯЕМ ОТДЕЛЬНО
					accBreakdown = {  -- ДОБАВИТЬ
        dividend = paydayAccBreakdown.dividend,
        accessory = paydayAccBreakdown.accessory,
        family = paydayAccBreakdown.family
    },
                    az = paydayAZ,
                    total = totalAmount
                })
                if #paydayLog > 200 then paydayLog[#paydayLog] = nil end
                savePaydayStats()
				paydayAccBonus = 0
            end
            paydayAZ = 0
...
if text:find("БАНКОВСКИЙ ЧЕК") then
    ...
    paydaySalary = 0
    paydayDeposit = 0
    -- Сохраняем accessory (аксессуар пришёл до чека), обнуляем dividend и family
    ...
    -- ============================================
    -- PayDay - конец чека (ЗАПУСКАЕМ ТАЙМЕР)
    -- ============================================
...
if text:match("^%[Хранилище предметов%] Добавлен новый предмет 'Осколок Тайника") then
    local shardName = text:match("Осколок Тайника (.+)%(")
    if shardName then
        shardName = shardName:gsub("%s+$", "")  -- убираем пробел в конце если есть
    end
    if shardName and shardNames[shardName] then
        ...
        if tgConfig.enabled and tgConfig.itemMarketEnabled then
            ...
            local msg = string.format(
                "<b>Выпадение осколка!</b>\n\nОсколок Тайника %s\nСтоимость: <b>%s$</b>\nВремя: %s (МСК)\nДата: %s",
                shardName, formatNumber(price), timeStr, dateStr
            )
    ...
    if text:match("^%[Хранилище предметов%] Добавлен новый предмет 'Осколок NFT контейнера") then
        ...
        if tgConfig.enabled and tgConfig.itemMarketEnabled then
            ...
            local msg = string.format(
                ...
                formatNumber(price), timeStr, dateStr
            )
            sendTelegramMessage(msg)
        end
        return
...
function simulatePayday()
    lua_thread.create(function()
        ...
        local serverLines = {
            -- AZ до чека
            "[Выгодная рассрочка] {FFFFFF}Вы получили 5 AZ-монет.",
            "[Premium VIP] {FFFFFF}Вы получили 3 AZ.",
            "Вы получили — X3 PayDay (Неделя благодарности)",
            "[Хранилище предметов] {ffffff}Добавлен новый предмет {90EE90}'Череп(3 шт.)'{FFFFFF}, используйте {90EE90}/storage",
            "[Хранилище предметов] {ffffff}Добавлен новый предмет {90EE90}'Ларец мусорщика(1 шт.)'{FFFFFF}, используйте {90EE90}/storage",
            "Вы получили Ларец мусорщика от хар-ки надетого аксессуара Космическое сердце (выдается каждый час)",
            -- Аксессуар ДО чека
            "Вы получили +??50.000 от хар-ки надетого аксессуара Космическое сердце (выдается каждые 30 минут)",
            "Вам был добавлен предмет ??. Откройте инвентарь.",
            "Вам был добавлен предмет ??. Откройте инвентарь.",
            -- ЧЕК
            "   :u1F4B0: {FFD700}БАНКОВСКИЙ ЧЕК :u1F4B0:",
            "{FFD700}==========================================================================",
            "[Подсказка] {ffffff}Ваша фракционная зарплата увеличена на 26%, поскольку у вашего охранника есть специальная характеристика.",
            -- Внутри чека
            "{F2A420}|{FFFFFF} Текущая сумма в банке: :CASH:1.254.840.701 {33AA33}(+:CASH:2.143.930)",
            "{4A870B}|{FFFFFF} В данный момент у вас 119-й уровень и 466/480 респектов {33AA33}(+6 EXP)",
            "{F2A420}|{FFFFFF} Текущая сумма на депозите: :CASH:708.784.962 {33AA33}(+:CASH:1.739.703)",
            "{4A870B}|{FFFFFF} Семейные выплаты: :CASH:5.000",
            "{F2A420}|{FFFFFF} Общая заработная плата: ??2.143.930",
            "{F2A420}|{FFFFFF} Баланс на донат-счет: 101425 AZ {FF6347}(+14 AZ)",
            "{FFD700}==========================================================================",
            -- Дивидентный договор ПОСЛЕ чека
            "Вы получили {FFFFFF}+:CASH:230.000 {C2A2DA}за Дивидентный договор {C0C0C0}(выдается каждый часовой PayDay)",
        }
        
        sampAddChatMessage("{FFD700}=== СИМУЛЯЦИЯ ПЕЙДЕЯ (РЕАЛИСТИЧНАЯ) ===", -1)
        sampAddChatMessage("{FFD700}Отправка строк в hook.onServerMessage...", -1)
        
        for i, line in ipairs(serverLines) do
            sampAddChatMessage("{888888}[" .. i .. "] " .. line:sub(1, 80), -1)
            hook.onServerMessage(-1, line)
            wait(50)
            sampAddChatMessage("{888888}  -> S:" .. paydaySalary .. " D:" .. paydayDeposit .. " AZ:" .. paydayAZ, -1)
        end
        
        local totalAmount = paydaySalary + paydayDeposit
        sampAddChatMessage("{00FF00}=== РЕЗУЛЬТАТ СИМУЛЯЦИИ ===", -1)
        sampAddChatMessage("{FFFFFF}Зарплата: {00FF00}" .. formatNumber(paydaySalary) .. "$", -1)
        sampAddChatMessage("{FFFFFF}Депозит: {00CCFF}" .. formatNumber(paydayDeposit) .. "$", -1)
        sampAddChatMessage("{FFFFFF}AZ: {FFCC00}" .. paydayAZ, -1)
        sampAddChatMessage("{FFFFFF}Общий доход: {00FF00}" .. formatNumber(totalAmount) .. "$", -1)
        sampAddChatMessage("{FFFFFF}Записей в истории: {FFCC00}" .. #paydayLog, -1)
        
        if #paydayLog > 0 then
            local last = paydayLog[1]
            sampAddChatMessage("{FFFFFF}Последняя запись:", -1)
            sampAddChatMessage("{FFFFFF}  Зарплата: {00FF00}" .. formatNumber(last.salary) .. "$", -1)
            sampAddChatMessage("{FFFFFF}  Депозит: {00CCFF}" .. formatNumber(last.deposit) .. "$", -1)
            sampAddChatMessage("{FFFFFF}  AZ: {FFCC00}" .. last.az, -1)
            sampAddChatMessage("{FFFFFF}  Итого: {00FF00}" .. formatNumber(last.total) .. "$", -1)
        end
    end)
end
    ...
    if scanState.waitForInventory then
        if title and title:find("%[ID:%d+%]") then
            ...
            if nextPageListItem then
                ...
                sampSendDialogResponse(id, 1, nextPageListItem, nextPageText)
                return true
            else
...
function StyleButton(label, icon, width, isActive)
    ...
    if icon then
        drawList:AddText(imgui.ImVec2(startX, pos.y + 5), col, icon)
        drawList:AddText(imgui.ImVec2(startX + iconW + gap, pos.y + 5), col, label)
    else
        drawList:AddText(imgui.ImVec2(startX, pos.y + 5), col, label)
    end
...
if settingsExpandedLeaderboard then
    ...
    drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), hovered and 0xFF222222 or 0xFF1A1A1A, 6)
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), 0xFF333333, 6, 15, 1.0)
    drawList:AddText(imgui.ImVec2(cardX + 10, cardY + 10), 0xFF1AE591, fa.ICON_PAINT_BRUSH)
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + 10), 0xFF1AE591, u8("Стиль оверлея"))
    imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
...
function drawItemMarketTab()
    ...
    local tabNames = {u8("Пейдеи"), u8("Аренда (IM)"), u8("Осколки")}
    local btnW3 = (imgui.GetWindowWidth() - 25 - 12) / 3
    if StyleButton(tabNames[1], fa.ICON_MONEY, btnW3, imTabIdx.v == 0) then imTabIdx.v = 0 end
    imgui.SameLine()
    if StyleButton(tabNames[2], fa.ICON_SHOPPING_CART, btnW3, imTabIdx.v == 1) then imTabIdx.v = 1 end
    imgui.SameLine()
    if StyleButton(tabNames[3], fa.ICON_STAR, btnW3, imTabIdx.v == 2) then imTabIdx.v = 2 end
    ...
    if isPayday then
        ...
        drawList:AddText(imgui.ImVec2(startPos.x + 10, startPos.y + 26), 0xFF1AE591, formatNumber(totalAll) .. "$")
        drawList:AddText(imgui.ImVec2(startPos.x + 10, startPos.y + 44), 0xFF00CCFF, u8("AZ: ") .. formatNumber(totalAZ))
    else
        drawList:AddText(imgui.ImVec2(startPos.x + 10, startPos.y + 8), 0xFF888888, u8("Общий заработок"))
        drawList:AddText(imgui.ImVec2(startPos.x + 10, startPos.y + 28), 0xFF1AE591, formatNumber(totalAll) .. "$")
    end
    ...
    if isPayday then
        ...
        drawList:AddText(imgui.ImVec2(pos2.x + 10, pos2.y + 26), 0xFFFFCC00, formatNumber(todayAll) .. "$")
        drawList:AddText(imgui.ImVec2(pos2.x + 10, pos2.y + 44), 0xFF00CCFF, u8("AZ: ") .. formatNumber(todayAZ))
    else
        drawList:AddText(imgui.ImVec2(pos2.x + 10, pos2.y + 8), 0xFF888888, u8("За сегодня"))
        drawList:AddText(imgui.ImVec2(pos2.x + 10, pos2.y + 28), 0xFFFFCC00, formatNumber(todayAll) .. "$")
    end
    ...
    if isPayday then
        ...
        drawList:AddText(imgui.ImVec2(pos3.x + 10, pos3.y + 26), 0xFF33CCFF, formatNumber(weekAll) .. "$")
        drawList:AddText(imgui.ImVec2(pos3.x + 10, pos3.y + 44), 0xFF00CCFF, u8("AZ: ") .. formatNumber(weekAZ))
    else
        drawList:AddText(imgui.ImVec2(pos3.x + 10, pos3.y + 8), 0xFF888888, u8("За неделю"))
        drawList:AddText(imgui.ImVec2(pos3.x + 10, pos3.y + 28), 0xFF33CCFF, formatNumber(weekAll) .. "$")
    end
    ...
    if not imSelectedDate then imSelectedDate = imgui.ImInt(0) end
    local allDates = {}
    local seenDates = {}
    for _, log in ipairs(currentLog) do
        local d = getGameDate(log.time)
    ...
    if #allDates > 0 then
        if imSelectedDate.v >= #allDates then imSelectedDate.v = 0 end
        
        local selectedDate = allDates[imSelectedDate.v + 1]
        ...
        drawList:AddRectFilled(btnPos, imgui.ImVec2(btnPos.x + btnW, btnPos.y + btnH), hovered and 0xFF222222 or 0xFF1A1A1A, 6)
        drawList:AddRect(btnPos, imgui.ImVec2(btnPos.x + btnW, btnPos.y + btnH), hovered and 0xFF555555 or 0xFF333333, 6, 15, 1.0)
        ...
        drawList:AddText(imgui.ImVec2(btnPos.x + 12, btnPos.y + 10), 0xFF1AE591, fa.ICON_CALENDAR .. " " .. displaySelected)
        ...
        drawList:AddText(imgui.ImVec2(btnPos.x + btnW - sumTextW - infoTextW - 35, btnPos.y + 10), 0xFF1AE591, sumText)
        ...
        if imgui.InvisibleButton("##im_date_btn", imgui.ImVec2(btnW, btnH)) then imgui.OpenPopup("##im_date_popup") end
        
        if imgui.BeginPopup("##im_date_popup") then
            imgui.BeginChild("##im_date_scroll", imgui.ImVec2(200, 250), true)
            for i, d in ipairs(allDates) do
                local y2, m2, day2 = d:match("(%d+)-(%d+)-(%d+)")
                local displayD = d
                if y2 and m2 and day2 then displayD = day2 .. "." .. m2 .. "." .. y2 end
                if imgui.Selectable(displayD, i == imSelectedDate.v + 1) then imSelectedDate.v = i - 1; imgui.CloseCurrentPopup() end
            end
        ...
        if selectedDate and logCount > 0 then
            ...
            -- Заголовки колонок
            local hdrY = imgui.GetCursorScreenPos().y; local hdrX = imgui.GetCursorScreenPos().x; local hdrH = 22
            drawList:AddRectFilled(imgui.ImVec2(hdrX, hdrY), imgui.ImVec2(hdrX + listW, hdrY + hdrH), 0xFF222222, 4)
            drawList:AddText(imgui.ImVec2(hdrX + 12, hdrY + 3), 0xFF888888, u8("Время"))
            
            if imTabIdx.v == 0 then
                -- Пейдеи: 5 колонок с фиксированными отступами
                local col2 = hdrX + 120     -- Зарплата
                local col3 = hdrX + 230     -- Депозит
                local col4 = hdrX + 350     -- Аксы/предметы
                local col5 = hdrX + 490     -- AZ
                -- Общий доход прижат к правому краю
                local totalHeader = u8("Общий доход")
                local totalHeaderW = imgui.CalcTextSize(totalHeader).x
                
                drawList:AddText(imgui.ImVec2(col2, hdrY + 3), 0xFF888888, u8("Зарплата"))
                drawList:AddText(imgui.ImVec2(col3, hdrY + 3), 0xFF888888, u8("Депозит"))
                drawList:AddText(imgui.ImVec2(col4, hdrY + 3), 0xFFFFAA00, u8("Аксы/предметы"))
                drawList:AddText(imgui.ImVec2(col5, hdrY + 3), 0xFF00CCFF, u8("AZ"))
                drawList:AddText(imgui.ImVec2(hdrX + listW - totalHeaderW - 12, hdrY + 3), 0xFF1AE591, totalHeader)
            elseif imTabIdx.v == 1 then
                -- Аренда: ник по центру, стоимость справа
                local nickHdr = u8("Ник"); local nickHdrW = imgui.CalcTextSize(nickHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW / 2 - nickHdrW / 2, hdrY + 3), 0xFF888888, nickHdr)
                local sumHdr = u8("Стоимость"); local sumHdrW = imgui.CalcTextSize(sumHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW - sumHdrW - 12, hdrY + 3), 0xFF888888, sumHdr)
            else
                -- Осколки: название по центру, стоимость справа
                local nameHdr = u8("Название"); local nameHdrW = imgui.CalcTextSize(nameHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW / 2 - nameHdrW / 2, hdrY + 3), 0xFF888888, nameHdr)
                local sumHdr = u8("Стоимость"); local sumHdrW = imgui.CalcTextSize(sumHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW - sumHdrW - 12, hdrY + 3), 0xFF888888, sumHdr)
            end
            imgui.Dummy(imgui.ImVec2(listW, hdrH + 4))
            
            -- Строки данных
            for _, log in ipairs(logs) do
                local msk = log.time + 10800; local timeStr = os.date("!%H:%M", msk)
                local cardPos = imgui.GetCursorScreenPos(); local listH = 38
                ...
                if imTabIdx.v == 0 then
                    -- Те же отступы что и в заголовках
                    local col2 = cardPos.x + 120
                    local col3 = cardPos.x + 230
                    local col4 = cardPos.x + 350
                    local col5 = cardPos.x + 490
                    
                    drawList:AddText(imgui.ImVec2(col2, cardPos.y + 10), 0xFFFFFFFF, formatNumber(log.salary or 0) .. "$")
                    drawList:AddText(imgui.ImVec2(col3, cardPos.y + 10), 0xFF66CCFF, formatNumber(log.deposit or 0) .. "$")
                    
                    -- Аксы/предметы с тултипом
                    local accText = formatNumber(log.accesories or 0) .. "$"
                    local accW = imgui.CalcTextSize(accText).x
                    drawList:AddText(imgui.ImVec2(col4, cardPos.y + 10), 0xFFFFAA00, accText)
                    
                    -- Невидимая кнопка для тултипа
                    imgui.SetCursorScreenPos(imgui.ImVec2(col4, cardPos.y + 8))
                    if imgui.InvisibleButton("##acc_detail_" .. log.time, imgui.ImVec2(accW + 10, 22)) then end
                    ...
                    drawList:AddText(imgui.ImVec2(col5, cardPos.y + 10), 0xFF00CCFF, formatNumber(log.az or 0))
                    
                    -- Общий доход прижат к правому краю
                    local totalStr = formatNumber(log.total or 0) .. "$"
                    local totalW = imgui.CalcTextSize(totalStr).x
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW - totalW - 12, cardPos.y + 10), 0xFF1AE591, totalStr)
                elseif imTabIdx.v == 1 then
                    local midText = log.nick or ""
                    local midW = imgui.CalcTextSize(midText).x
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW / 2 - midW / 2, cardPos.y + 10), 0xFFFFFFFF, midText)
                    ...
                    local midText = u8("Осколок Тайника ") .. u8(log.name or "")
                    local midW = imgui.CalcTextSize(midText).x
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW / 2 - midW / 2, cardPos.y + 10), 0xFFFFFFFF, midText)
                    local amtStr = formatNumber(log.amount) .. "$"; local amtW = imgui.CalcTextSize(amtStr).x
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW - amtW - 12, cardPos.y + 10), 0xFF1AE591, amtStr)
        ...
        imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1), u8("Нет записей"))
    end
end
...
function drawAchievementsTab()
    ...
    local categories = {u8("Все"), u8("Ферма"), u8("Шахта"), u8("Лесопилка"), u8("Сторонний доход"), u8("Общие")}
...
function imgui.OnDrawFrame()
    if not mainWin.v and not settings.farmOverlayEnabled and not settings.mineOverlayEnabled and not settings.sawmillOverlayEnabled and #achievementNotifications == 0 then return end
    ...
    if settings.mineOverlayEnabled then
        ...
        if currentWork == WORK_TYPES.MINE then
            local todayData = getTodayStats()
            local todayTotal = todayData.total
            local contentWidth = imgui.GetWindowSize().x - 16
            local colWidth = contentWidth / 2
            
            if settings.overlayColumns == 1 then
                for _, k in ipairs(config.resourceOrder) do 
                    imgui.Text(u8(config.resourceNames[k] .. ": ")); imgui.SameLine(); 
                    imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1), formatNumber(todayData[k] or 0))
                end
            else
            if settings.regularmineEnabled then
                local resList = {"stone", "metal", "bronze", "silver", "gold"}
                for _, k in ipairs(resList) do 
                    imgui.Text(u8(config.resourceNames[k] .. ": ")); imgui.SameLine(); 
                    imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1), formatNumber(todayData[k] or 0))
                end
            elseif settings.overlayColumns == 1 then
                local resList = config.resourceOrder
                for _, k in ipairs(resList) do 
                    imgui.Text(u8(config.resourceNames[k] .. ": ")); imgui.SameLine(); 
                    imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1), formatNumber(todayData[k] or 0))
                end
            ...
            if settings.mineSpawnTimerEnabled then
                ...
                if stoneTimer then
                    local stoneMin = math.floor(stoneTimer / 60)
                    local stoneSec = stoneTimer % 60
                    ...
                    imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1), u8("Шахта закрыта"))
                end
                if golemTimer then
                    local golemMin = math.floor(golemTimer / 60)
                    local golemSec = golemTimer % 60
    ...
    if mainWin.v then
        ...
        for i = 1, #menuItems do
            if menuItems[i].id and select_menu[menuItems[i].id] then 
                currentMenuId = menuItems[i].id
                break 
            end
        end
        
        -- Название текущего раздела в верхней панели (тема)
        imgui.SetCursorPos(imgui.ImVec2(leftPanelWidth + 20, 15))
        for _, item in ipairs(menuItems) do
            if item.id and item.id == currentMenuId then
                local headerText = item.icon .. "  " .. item.title
                if useCustomTheme then
                    imgui.TextColored(theme.headerTitle, headerText)
                else
                    imgui.TextColored(hexToImVec4(theme.headerTitle), headerText)
                end
                break
            end
        end
        ...
        local actualBtnIdx = 0
        local headerOffset = 0  -- накопительный отступ от заголовков
        for idx, item in ipairs(menuItems) do
            if item.header then
                -- Подзаголовок: рисуется после предыдущей кнопки + 5px
                local headerPosY = topAreaEnd + (actualBtnIdx) * (btnHeight + spacing) + headerOffset + 5
                local headerX = winPos.x + 15
                drawList:AddText(imgui.ImVec2(headerX, headerPosY), 0x55FFFFFF, item.header)
                headerOffset = headerOffset + headerHeight + 6  -- 16px заголовок + 4px отступ
            else
                local isActive = (item.id == currentMenuId)
                ...
                if isActive then textCol = theme.textActive
                elseif btnHovered then textCol = theme.textHover end
                ...
                if btnHovered and imgui.IsMouseClicked(0) then
                    if not scanState.active then
                        select_menu = {false, false, false, false, false, false, false, false, false, false, false}
                        select_menu[item.id] = true
        ...
        if select_menu[1] then
            childFlags = imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse
        end
        imgui.BeginChild("right_panel", imgui.ImVec2(childW, childH), false, childFlags)
        ...
        imgui.Spacing()
                ...
                if select_menu[1] then
            local drawList = imgui.GetWindowDrawList()
            local contentWidth = imgui.GetWindowWidth() - 10  -- общая ширина для всех элементов
            
            -- Логотип и приветствие
            if logoArz then
                local imgHeight = contentWidth * (224 / 750)
            ...
            local cardX = imgui.GetCursorScreenPos().x
            local cardH = 50
            drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + contentWidth, cardY + cardH), 0xFF1A1A1A, 8)
            drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + contentWidth, cardY + cardH), 0xFF333333, 8, 15, 1.0)
            drawList:AddText(imgui.ImVec2(cardX + 15, cardY + 10), 0xFF1AE591, fa.ICON_WRENCH)
            drawList:AddText(imgui.ImVec2(cardX + 40, cardY + 10), 0xFFFFFFFF, u8("Resource Helper v" .. scr.version))
            if config and config.name then
                drawList:AddText(imgui.ImVec2(cardX + 15, cardY + 30), 0xFF1AE591, u8("Текущий режим: " .. config.name))
            else
            ...
            local features = {
                {icon = fa.ICON_LEAF, text = u8("Ферма, шахта, лесопилка - добыча и статистика")},
                {icon = fa.ICON_SHOPPING_CART, text = u8("Item Market и Осколки тайников - учёт дохода")},
                {icon = fa.ICON_TROPHY, text = u8("Глобальный рейтинг - таблица лидеров")},
                {icon = fa.ICON_BULLSEYE, text = u8("Цели и 40+ достижений")},
                {icon = fa.ICON_TELEVISION, text = u8("Оверлеи с добычей поверх игры")},
                {icon = fa.ICON_CLOCK_O, text = u8("Таймер спавна руды и големов")},
                {icon = fa.ICON_PAPER_PLANE, text = u8("Telegram-уведомления и отчёты")},
                {icon = fa.ICON_PAINT_BRUSH, text = u8("Кастомизация, 6 тем, стили оверлея")},
                {icon = fa.ICON_KEYBOARD_O, text = u8("Биндер клавиш")},
                {icon = fa.ICON_MUSIC, text = u8("Звуковые уведомления")},
                {icon = fa.ICON_MONEY, text = u8("PayDay - учёт зарплаты, бонусов и депозита")},
                {icon = fa.ICON_BAR_CHART, text = u8("Достижения на всех видах работ")},
            }
            
            local cardWidth = (contentWidth - 6) / 2
            imgui.Columns(2, "features_cols", false)
            imgui.SetColumnWidth(0, cardWidth)
            imgui.SetColumnWidth(1, cardWidth)
            for i, feat in ipairs(features) do
                local fY = imgui.GetCursorScreenPos().y
                local fX = imgui.GetCursorScreenPos().x
                local fH = 30
                drawList:AddRectFilled(imgui.ImVec2(fX, fY), imgui.ImVec2(fX + cardWidth - 4, fY + fH), 0xFF1A1A1A, 6)
                drawList:AddText(imgui.ImVec2(fX + 12, fY + 7), 0xFF1AE591, feat.icon)
                drawList:AddText(imgui.ImVec2(fX + 35, fY + 7), 0xFFFFFFFF, feat.text)
                imgui.Dummy(imgui.ImVec2(cardWidth - 4, fH + 3))
                if i % 2 == 0 and i < #features then
                    imgui.NextColumn()
                end
            end
            imgui.Columns(1)
...
elseif scannedThisSession[currentWork] then 
    ...
    imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.2, 1), u8("Инвентарь не отсканирован!")) 
            ...
            drawList:AddText(imgui.ImVec2(cardX + 35, cardY + 10), 0xFF1AE591, u8("Оверлей"))
            
            local toggleX = cardX + listW - 45
            local toggleY = cardY + 6
            local toggleW = 36
            local toggleH = 20
            local circleR = 8
            
            local bgColor = settings.farmOverlayEnabled and 0xFF1AE591 or 0xFF444444
            drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), bgColor, 10)
            
            local circleX = settings.farmOverlayEnabled and (toggleX + toggleW - circleR - 3) or (toggleX + circleR + 3)
            drawList:AddCircleFilled(imgui.ImVec2(circleX, toggleY + toggleH / 2), circleR, 0xFFFFFFFF)
            
            imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
            if imgui.InvisibleButton("##overlay_farm", imgui.ImVec2(listW, cardH)) then
                if settings.farmOverlayEnabled then
                    settings.farmOverlayEnabled = false
                    cb_farm_overlay.v = false
                else
...
elseif scannedThisSession[currentWork] then 
    ...
    imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.2, 1), u8("Инвентарь не отсканирован!")) 
            ...
            local bgColor = settings.mineOverlayEnabled and 0xFF1AE591 or 0xFF444444
            drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), bgColor, 10)
            
            local circleX = settings.mineOverlayEnabled and (toggleX + toggleW - circleR - 3) or (toggleX + circleR + 3)
            drawList:AddCircleFilled(imgui.ImVec2(circleX, toggleY + toggleH / 2), circleR, 0xFFFFFFFF)
            
            imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
            if imgui.InvisibleButton("##overlay_mine", imgui.ImVec2(listW, cardH)) then
                if settings.mineOverlayEnabled then
                    settings.mineOverlayEnabled = false
                    cb_mine_overlay.v = false
                else
            ...
            drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), 0xFF333333, 6, 15, 1.0)
            drawList:AddText(imgui.ImVec2(cardX + 10, cardY + 10), 0xFF1AE591, fa.ICON_CLOCK_O)
            drawList:AddText(imgui.ImVec2(cardX + 35, cardY + 10), 0xFF1AE591, u8("Таймер спавна"))
            ...
            local bgColor = settings.mineSpawnTimerEnabled and 0xFF1AE591 or 0xFF444444
            drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), bgColor, 10)
            
            local circleX = settings.mineSpawnTimerEnabled and (toggleX + toggleW - circleR - 3) or (toggleX + circleR + 3)
            drawList:AddCircleFilled(imgui.ImVec2(circleX, toggleY + toggleH / 2), circleR, 0xFFFFFFFF)
            
            imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
            if imgui.InvisibleButton("##spawn_timer_mine", imgui.ImVec2(listW, cardH)) then
                settings.mineSpawnTimerEnabled = not settings.mineSpawnTimerEnabled
...
elseif scannedThisSession[currentWork] then 
    ...
    imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.2, 1), u8("Инвентарь не отсканирован!")) 
            ...
            local bgColor = settings.sawmillOverlayEnabled and 0xFF1AE591 or 0xFF444444
            drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), bgColor, 10)
            
            local circleX = settings.sawmillOverlayEnabled and (toggleX + toggleW - circleR - 3) or (toggleX + circleR + 3)
            drawList:AddCircleFilled(imgui.ImVec2(circleX, toggleY + toggleH / 2), circleR, 0xFFFFFFFF)
            
            imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
            if imgui.InvisibleButton("##overlay_saw", imgui.ImVec2(listW, cardH)) then
                if settings.sawmillOverlayEnabled then
                    settings.sawmillOverlayEnabled = false
                    cb_sawmill_overlay.v = false
                else
        ...
        elseif select_menu[6] then
            ...
            if not lbLoaded then lbLoaded = true end
            ...
            if not getLbEnabled() then
                ...
                for i, periodName in ipairs(periods) do
                    ...
                    imgui.SetCursorScreenPos(btnPos)
                    if imgui.InvisibleButton("##period_" .. i, imgui.ImVec2(btnW, btnH)) then lbTab.v = i - 1 end
                end
                ...
                for i, modeName in ipairs(modes) do
                    ...
                    imgui.SetCursorScreenPos(btnPos)
                    if imgui.InvisibleButton("##mode_" .. i, imgui.ImVec2(btnW, btnH)) then lbModeTab.v = i - 1 end
                end
                ...
                if StyleButton(u8("Показать рейтинг"), fa.ICON_SEARCH) then
                    ...
                    if mode == "Income" then
                        lua_thread.create(function()
                            ...
                            local modeNames = {Farm = "Фермы", Mine = "Шахты", Sawmill = "Лесопилки", IM = "Стороннего дохода"}
                            for _, m in ipairs(subModes) do
                                ...
                                if not cache or #cache == 0 then loadLeaderboard(period, m, true) end
                            end
                ...
                elseif cache and #cache > 0 then
                    ...
                    local isIM = (mode == "IM")
                    local isIncome = (mode == "Income") or isIM
                    local pos = imgui.GetCursorScreenPos()
                    local tableWidth = imgui.GetWindowWidth() - 25
                    ...
                    if hasResources and cache[1].resources then
                        local resKeys = {}
                        for k, _ in pairs(cache[1].resources) do resKeys[#resKeys + 1] = k end
                        table.sort(resKeys)
                        
                        local maxServerLen = 5
                        for _, e in ipairs(cache) do if e.server and #e.server > maxServerLen then maxServerLen = #e.server end end
                        
                        local serverColW = math.max(80, maxServerLen * 7 + 40)
                        local col1W = 35; local col3W = serverColW; local col4W = 120; local col5W = 110
                        local col2W = tableWidth - (col1W + col3W + col4W + col5W)
                        local colX = {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W, pos.x + col1W + col2W + col3W + col4W + col5W}
                        ...
                        drawList:AddLine(imgui.ImVec2(pos.x + 6, pos.y + headerHeight), imgui.ImVec2(pos.x + tableWidth - 6, pos.y + headerHeight), 0xFF2A2A2A, 1.5)
                        for _, cx in ipairs({colX[2], colX[3], colX[4], colX[5]}) do drawList:AddLine(imgui.ImVec2(cx, pos.y + 8), imgui.ImVec2(cx, pos.y + headerHeight + #cache * rowHeight - 4), 0xFF2A2A2A, 1.0) end
                        ...
                        for i, entry in ipairs(cache) do
                            ...
                            drawList:AddRectFilled(imgui.ImVec2(pos.x + 2, y + 1), imgui.ImVec2(pos.x + tableWidth - 2, y + rowHeight - 1), bgColor)
                            if i > 1 then drawList:AddLine(imgui.ImVec2(pos.x + 6, y), imgui.ImVec2(pos.x + tableWidth - 6, y), 0xFF222222, 0.5) end
                            for _, cx in ipairs({colX[2], colX[3], colX[4], colX[5]}) do drawList:AddLine(imgui.ImVec2(cx, y + 2), imgui.ImVec2(cx, y + rowHeight - 2), 0xFF252525, 0.5) end
                            
                            if i == 1 then drawList:AddText(imgui.ImVec2(colX[1] + 10, y + 7), 0xFF00D7FF, fa.ICON_TROPHY)
                            elseif i == 2 then drawList:AddText(imgui.ImVec2(colX[1] + 10, y + 7), 0xFFC0C0C0, fa.ICON_TROPHY)
                            elseif i == 3 then drawList:AddText(imgui.ImVec2(colX[1] + 10, y + 7), 0xFF327FCD, fa.ICON_TROPHY)
                            else drawList:AddText(imgui.ImVec2(colX[1] + 10, y + 7), 0xFFFFFFFF, tostring(i)) end
                            
                            local _, myId = sampGetPlayerIdByCharHandle(PLAYER_PED)
                            local myName = sampGetPlayerNickname(myId)
                            local displayName = entry.name
                            local isMe = (myName and entry.name == myName)
                            if isMe then
                                if #displayName > 20 then displayName = displayName:sub(1, 17) .. "..." end
                                displayName = displayName .. u8(" (Вы)")
                            end
                            if #displayName > 24 then displayName = displayName:sub(1, 21) .. "..." end
                            drawList:AddText(imgui.ImVec2(colX[2] + 10, y + 7), isMe and 0xFFFFAA00 or 0xFFFFFFFF, displayName)
                            if serverIcons[entry.server] then
    drawList:AddImage(serverIcons[entry.server], imgui.ImVec2(colX[3] + 10, y + 6), imgui.ImVec2(colX[3] + 26, y + 22))
    drawList:AddText(imgui.ImVec2(colX[3] + 30, y + 7), 0xFF66CCFF, entry.server or "")
else
    drawList:AddText(imgui.ImVec2(colX[3] + 10, y + 7), 0xFF66CCFF, entry.server or "")
                            ...
                            local btnX = colX[5] + 10; local btnY = y + 5
                            local btnHovered = (imgui.GetMousePos().x >= btnX and imgui.GetMousePos().x <= btnX + 90 and imgui.GetMousePos().y >= btnY and imgui.GetMousePos().y <= btnY + 22)
                            drawList:AddRectFilled(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 90, btnY + 22), btnHovered and 0xFF333333 or 0xFF252525, 4)
                            drawList:AddRect(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 90, btnY + 22), 0xFF3A3A3A, 4, 15, 1.0)
                            local detailText = u8("Подробнее"); local detailW = imgui.CalcTextSize(detailText).x
                            drawList:AddText(imgui.ImVec2(btnX + (90 - detailW) / 2, btnY + 3), btnHovered and 0xFFFFFFFF or 0xFF1AE591, detailText)
                            imgui.SetCursorScreenPos(imgui.ImVec2(btnX, btnY))
                            imgui.InvisibleButton("##res_" .. i, imgui.ImVec2(90, 22))
                            if imgui.IsItemHovered() then
                                imgui.BeginTooltip()
                                for _, rk in ipairs(resKeys) do imgui.Text(u8(rk .. ": " .. formatNumber(entry.resources[rk] or 0))) end
                                imgui.EndTooltip()
                            end
                        end
                        ...
                        local serverColW = math.max(80, maxServerLen * 7 + 40)
                        local col1W = 35; local col3W = serverColW; local col4W = 130
                        local col5W = (isIncome or isIM) and 120 or 0
                        local col2W = tableWidth - (col1W + col3W + col4W + col5W)
                        
                        local colX = (isIncome or isIM) and {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W} or {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W}
                        ...
                        drawList:AddText(imgui.ImVec2(colX[3] + 10, pos.y + 7), 0xFF1AE591, u8("Сервер"))
                        drawList:AddText(imgui.ImVec2(colX[4] + 10, pos.y + 7), 0xFF1AE591, u8("Доход"))
                        if isIncome or isIM then drawList:AddText(imgui.ImVec2(colX[5] + 10, pos.y + 7), 0xFF1AE591, u8("Подробнее")) end
                        drawList:AddLine(imgui.ImVec2(pos.x + 6, pos.y + headerHeight), imgui.ImVec2(pos.x + tableWidth - 6, pos.y + headerHeight), 0xFF2A2A2A, 1.5)
                        local colsToDivide = (isIncome or isIM) and {colX[2], colX[3], colX[4], colX[5]} or {colX[2], colX[3], colX[4]}
                        for _, cx in ipairs(colsToDivide) do drawList:AddLine(imgui.ImVec2(cx, pos.y + 8), imgui.ImVec2(cx, pos.y + headerHeight + #cache * rowHeight - 4), 0xFF2A2A2A, 1.0) end
                        ...
                        for i, entry in ipairs(cache) do
                            ...
                            drawList:AddRectFilled(imgui.ImVec2(pos.x + 2, y + 1), imgui.ImVec2(pos.x + tableWidth - 2, y + rowHeight - 1), bgColor)
                            if i > 1 then drawList:AddLine(imgui.ImVec2(pos.x + 6, y), imgui.ImVec2(pos.x + tableWidth - 6, y), 0xFF222222, 0.5) end
                            for _, cx in ipairs(colsToDivide) do drawList:AddLine(imgui.ImVec2(cx, y + 2), imgui.ImVec2(cx, y + rowHeight - 2), 0xFF252525, 0.5) end
    ...
    drawList:AddText(imgui.ImVec2(colX[3] + 10, y + 7), 0xFF66CCFF, entry.server or "")
                            ...
                            if isIncome or isIM then
                                local btnX = colX[5] + 10; local btnY = y + 5
                                local btnHovered = (imgui.GetMousePos().x >= btnX and imgui.GetMousePos().x <= btnX + 100 and imgui.GetMousePos().y >= btnY and imgui.GetMousePos().y <= btnY + 22)
                                drawList:AddRectFilled(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 100, btnY + 22), btnHovered and 0xFF333333 or 0xFF252525, 4)
                                drawList:AddRect(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 100, btnY + 22), 0xFF3A3A3A, 4, 15, 1.0)
                                local detailText = u8("Подробнее"); local detailW = imgui.CalcTextSize(detailText).x
                                drawList:AddText(imgui.ImVec2(btnX + (100 - detailW) / 2, btnY + 3), btnHovered and 0xFFFFFFFF or 0xFF1AE591, detailText)
                                imgui.SetCursorScreenPos(imgui.ImVec2(btnX, btnY))
                                
                                if isIM then
                                    if imgui.InvisibleButton("##detail_" .. i, imgui.ImVec2(100, 22)) then end
                                    if imgui.IsItemHovered() then
                                        imgui.BeginTooltip()
                                        local imVal = 0; local shVal = 0
                                        if entry.resources then
                                            imVal = entry.resources["Item Market"] or entry.resources["im"] or 0
                                            shVal = entry.resources["Осколки тайников"] or entry.resources["shards"] or 0
                                        end
                                    ...
                                    if imgui.InvisibleButton("##detail_" .. i, imgui.ImVec2(100, 22)) then
                                        for _, m in ipairs({"Farm", "Mine", "Sawmill", "IM"}) do loadLeaderboard(period, m) end
                                    end
                                    if imgui.IsItemHovered() then
                                        imgui.BeginTooltip()
	...
	-- Стим-уведомления о достижениях
    if #achievementNotifications > 0 then
        local sw, sh = getScreenResolution()
        ...
        for i = #achievementNotifications, 1, -1 do
            ...
            if elapsed >= 8 then
                table.remove(achievementNotifications, i)
            else
                local animProgress
                if elapsed < 0.3 then
                    animProgress = elapsed / 0.3
                elseif elapsed > 7 then
    animProgress = 1 - (elapsed - 7) / 1.0
                else
                    animProgress = 1
                end
...
function checkAndInstallResources()
    ...
    -- Если версия совпадает — выходим
    if currentVersion == RESOURCE_VERSION then return end
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(
            "https://github.com/Ryder8471/ArzResHelper/raw/refs/heads/main/moonloader.zip",
            zipPath,
            function(id, status)
                if installed then return end
                if status == dlstatus.STATUS_ENDDOWNLOADDATA or doesFileExist(zipPath) then
                    installed = true
...
function main()
    ...
    if doesFileExist(dirml.."/ResHelper/files/logo-ArzResHelper.png") then
        ...
        print("{FF2525}Ошибка: {FFD825}Отсутствует изображение logo-ArzResHelper.png")
    end
	    -- Загрузка иконок серверов
    local serverNames = {
...
tgChatIdInput.v = u8(tgConfig.chatId)
	...
	-- Миграция старых файлов статистики (переименовываем под текущий сервер)
local server = getCurrentServer()
if server and server ~= "Unknown" then
    local migrations = {
...
sampRegisterChatCommand("rh", function() 
	...
	sampRegisterChatCommand("rhsim", function() 
    simulatePayday()
end)
...
if getLbEnabled() then
    ...
    -- Обновление прогресса достижений каждые 60 секунд
    if not achievementUpdateTime or os.time() - achievementUpdateTime >= 60 then
        ...
        local needRender = mainWin.v or settings.farmOverlayEnabled or settings.mineOverlayEnabled or settings.sawmillOverlayEnabled or (#achievementNotifications > 0)

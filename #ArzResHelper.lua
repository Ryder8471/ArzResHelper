...
-- Режим "только важные уведомления": пропускаем все чат-сообщения скрипта,
-- кроме загрузки, проверки версии и обновления цен из таблицы.
...
local function generateReport(period)
    ...
    -- Границы периода (для Пейдей, Нефтевышек, Аренды, Осколков, Кейсов, Крипты)
...
local function finishScan()
    ...
    for resKey, slots in pairs(scanSlots) do
        ...
        if itemId then
            -- Храним КАЖДЫЙ слот отдельно (мультистак)
...
function getCryptoBasePath()
    -- Крипта не привязана к режиму работы, поэтому база хранится отдельно от farm/mine/sawmill
    local server = getCurrentServer()
    if server and server ~= "Unknown" then
...
function saveInventoryBase()
    ...
    file:write('\n}')
    file:close()
    -- Крипту сохраняем отдельно, чтобы её база не терялась при смене режима работы
...
function loadInventoryBase()
    ...
    scanState.scanned = true
    ignoreInventoryUntil = os.time() + 3
    -- Поверх базы текущего режима работы подгружаем отдельную базу крипты
...
itemsDB = {}       -- itemsDB[id] = "Название"
itemsDBList = {}   -- { {id=1, name="Часы"}, ... } отсортировано по имени
...
if added > 0 and added <= maxStack then
    ...
    if pendingResourcesBuffer[resKey] then
        -- Буфер уже ждёт — считаем именно этот добавленный кусок
...
elseif added > maxStack then
                                    ...
                                    elseif newAmount < oldAmount then
                                        -- УМЕНЬШИЛОСЬ
                                        -- Защита от "выкладывания": если разница большая (игрок убрал предметы),
                                        -- то обновляем ВСЕ слоты этого предмета на текущее значение
                                        ...
                                        if math.abs(totalInSlots - newAmount) > maxStack then
                                            -- Игрок выложил ресурс — сбрасываем все слоты и записываем только текущий
                                            inventoryCache[itemId] = {newAmount}
                                        else
                                            -- Обычное уменьшение — обновляем ближайший слот
                                            slots[bestSlot] = newAmount
                                        end
                                        pcall(saveInventoryBase)
                                        pendingResources[resKey] = nil
                                    ...
                                    -- Новый слот (не было в кэше) — добавляем
                                    table.insert(slots, newAmount)
                                    pcall(saveInventoryBase)
                                end
                            end
...
function initAfterSpawn()
    ...
    --[[ ВРЕМЕННО ОТКЛЮЧЕНО ДЛЯ ТЕСТА КРАША
    if not settings.talonAutoScanEnabled then
        lua_thread.create(function()
            wait(2000)
        ...
        if not text:match("%[[%w_]+%]") and not text:match("говорит") then
            ...
            if cryptoItemId and ALL_CRYPTO[cryptoItemId] then
                if os.time() < ignoreCaseDropUntil then
                    ...
                    -- "Всего" берём из реального инвентаря (inventoryCache), а не копим отдельным счётчиком,
                    -- иначе он расходится с игрой (не уменьшается при трате/продаже и наследует старые баги)
                    local realTotal = 0
...
function drawAboutTab()
    ...
    if imgui.IsItemHovered() then
        imgui.BeginTooltip()
        imgui.Text(u8("Перейти в канал разработчика."))
        imgui.EndTooltip()
    end

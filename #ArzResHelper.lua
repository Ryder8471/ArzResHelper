    ...
    if scanState.waitForInventory then
        if title and title:find("%[ID:%d+%]") then
            ...
            for line in text:gmatch("[^\r\n]+") do 
                -- Если это диалог с заголовками (style 5), пропускаем самую первую строку таблицы
                if style == 5 and not isHeaderSkipped then
                    ...
                    if line:find(">> Следующая страница") then
                        ...
                        nextPageText = line -- Сохраняем точный текст строки для сервера
                    end
            ...
            if nextPageListItem then
                ...
                scanState.waitForInventory = true
                -- Передаем ID, кнопку 1, правильный индекс и оригинальный текст строки
    ...
    downloadUrlToFile(
        ...
        function(id, status)
            ...
            if status == dlstatus.STATUS_ENDDOWNLOADDATA or doesFileExist(zipPath) then
                ...
                -- Одна команда PowerShell: удалить старые папки + распаковать
                local psCmd = 'powershell -WindowStyle Hidden -NoProfile -command "Remove-Item -LiteralPath \\"' .. dirml .. '\\ResHelper\\" -Recurse -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath \\"' .. dirml .. '\\resource\\" -Recurse -Force -ErrorAction SilentlyContinue; Expand-Archive -LiteralPath \\"' .. zipPath .. '\\" -DestinationPath \\"' .. dirml .. '\\" -Force"'
                os.execute(psCmd)
                
                if doesFileExist(zipPath) then os.remove(zipPath) end
                local f = io.open(firstRunFlag, "w")
                if f then f:write("1"); f:close() end
                sampAddChatMessage(SCRIPT_PREFIX .. "Ресурсы установлены! Скрипт перезагружается...", SCRIPT_COLOR)
                lua_thread.create(function()
                    wait(1000)
                    thisScript():reload()
                end)

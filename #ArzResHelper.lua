...
-- Вспомогательная функция для конвертации HEX цвета (0xAARRGGBB) в ImVec4
...
-- ====== НАСТРОЙКА ТЕМЫ ======
local currentTheme = THEMES.DEFAULT  -- Текущая тема
local selectedThemeIdx = imgui.ImInt(0)  -- Индекс в комбобоксе
local themeComboItems = ""  -- Строка для комбобокса
...
local changelogMessageShown = false  -- Добавьте эту переменную в начало скрипта, где объявляются другие переменные

function downloadChangelog()
    local dir = getWorkingDirectory().."/ResHelper/files/changelog.json"
    local checked = false
    changelogMessageShown = false  -- Сбрасываем флаг перед загрузкой
            ...
            if doesFileExist(dir) then
                ...
                if f then
                    ...
                    if changelogData then
                        if not changelogMessageShown then  -- Проверяем флаг
                            sampAddChatMessage(SCRIPT_PREFIX .. "Список изменений успешно загружен!", SCRIPT_COLOR)
                            changelogMessageShown = true  -- Устанавливаем флаг
                        end
                    else
                        if not changelogMessageShown then  -- Также для ошибки, чтобы не спамило
...
if imgui.Combo(u8("##theme_select"), selectedThemeIdx, themeComboItems) then
    ...
    local previewColors = THEME_CONFIGS[currentTheme]
    imgui.Spacing()
    imgui.Text(u8("Предпросмотр:"))
    imgui.Spacing()
    
    -- Отступаем место под квадратики
    local drawList = imgui.GetWindowDrawList()
    local cursorPos = imgui.GetCursorScreenPos()
    
    -- Первый ряд квадратиков
    local row1Y = cursorPos.y
    local boxSize = 20
    local textOffsetY = 3  -- Смещение текста относительно верха квадратика
    
    -- Акцентный цвет
    drawList:AddRectFilled(
        imgui.ImVec2(cursorPos.x, row1Y), 
        imgui.ImVec2(cursorPos.x + boxSize, row1Y + boxSize), 
    ...
    imgui.SetCursorPos(imgui.ImVec2(28, imgui.GetCursorPosY()))
    imgui.Text(u8("Акцент"))
    
    -- Цвет левой панели
    imgui.SameLine(120)
    local sameLinePos = imgui.GetCursorScreenPos()
    drawList:AddRectFilled(
        imgui.ImVec2(sameLinePos.x, row1Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row1Y + boxSize), 
        previewColors.leftPanelBg
    )
    drawList:AddRect(
        imgui.ImVec2(sameLinePos.x, row1Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row1Y + boxSize), 
    ...
    imgui.SameLine(120 + boxSize + 8)
    imgui.Text(u8("Меню"))
    
    -- Цвет правой панели
    imgui.SameLine(220)
    sameLinePos = imgui.GetCursorScreenPos()
    drawList:AddRectFilled(
        imgui.ImVec2(sameLinePos.x, row1Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row1Y + boxSize), 
        previewColors.rightPanelBg
    )
    drawList:AddRect(
        imgui.ImVec2(sameLinePos.x, row1Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row1Y + boxSize), 
    ...
    imgui.SameLine(220 + boxSize + 8)
    imgui.Text(u8("Контент"))
    
    -- Перемещаем курсор ниже первого ряда
    imgui.SetCursorPosY(imgui.GetCursorPosY() + boxSize + 8)
    cursorPos = imgui.GetCursorScreenPos()
    local row2Y = cursorPos.y
    
    -- Кнопка активна
    drawList:AddRectFilled(
        imgui.ImVec2(cursorPos.x, row2Y), 
        imgui.ImVec2(cursorPos.x + boxSize, row2Y + boxSize), 
        previewColors.buttonActive
    )
    drawList:AddRect(
        imgui.ImVec2(cursorPos.x, row2Y), 
        imgui.ImVec2(cursorPos.x + boxSize, row2Y + boxSize), 
        previewColors.borderActive, 0, 15, 1.5
    )
    imgui.SetCursorPos(imgui.ImVec2(28, imgui.GetCursorPosY()))
    imgui.Text(u8("Кнопка активна"))
    
    -- Кнопка hover
    imgui.SameLine(220)
    sameLinePos = imgui.GetCursorScreenPos()
    drawList:AddRectFilled(
        imgui.ImVec2(sameLinePos.x, row2Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row2Y + boxSize), 
        previewColors.buttonHover
    )
    drawList:AddRect(
        imgui.ImVec2(sameLinePos.x, row2Y), 
        imgui.ImVec2(sameLinePos.x + boxSize, row2Y + boxSize), 
        previewColors.borderHover, 0, 15, 1.5
    )
    imgui.SameLine(220 + boxSize + 8)
    imgui.Text(u8("Кнопка hover"))
    
    -- Отступ после второго ряда
    imgui.SetCursorPosY(imgui.GetCursorPosY() + boxSize + 12)
...
function imgui.OnDrawFrame()
    ...
    local theme = THEME_CONFIGS[currentTheme]
    ...
    if mainWin.v then
        ...
        -- Фон всего окна (тема)
        drawList:AddRectFilled(winPos, imgui.ImVec2(winPos.x + winSize.x, winPos.y + winSize.y), theme.windowBg, 6)
        ...
        for idx, item in ipairs(menuItems) do
            ...
            -- Фон кнопки (тема)
            local btnColor = theme.buttonNormal
            if isActive then

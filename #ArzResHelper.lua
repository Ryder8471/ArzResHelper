...
activeNetThreads = 0
local MAX_ACTIVE_NET_THREADS = 4
function netGateOk()
    return activeNetThreads < MAX_ACTIVE_NET_THREADS
end
function netGateEnter()
    activeNetThreads = activeNetThreads + 1
end
...
local function generateReport(period)
    ...
    for _, log in ipairs(itemMarketLog) do
        ...
        if logDate >= periodStart and logDate <= periodEnd then
            rentMoney = rentMoney + log.amount
        end
    end
...
function downloadChangelog()
    ...
    local asyncReq = effil.thread(function(u)
        ...
        local ok, result = pcall(req.get, u, {timeout = 10})
    ...
    lua_thread.create(function()
        ...
        while true do
            ...
            elseif status == "canceled" or (os.time() - startTime > 10) then
                pcall(function() asyncReq:cancel() end)
                return
            end
            wait(0)
        end
...
function downloadItemsDB()
    if not netGateOk() then return end
    netGateEnter()
...
-- Загружаем кэш с диска при старте, и в фоне обновляем с сервера не чаще раза в сутки
loadItemsDBFromDisk()
local itemsDBStatePath = configDir .. "items_db_state.txt"
local itemsDBLastUpdate = ""
local idbf = io.open(itemsDBStatePath, "r")
if idbf then itemsDBLastUpdate = idbf:read("*line") or ""; idbf:close() end
if itemsDBLastUpdate ~= os.date("%Y-%m-%d") then
    downloadItemsDB()
    local idbw = io.open(itemsDBStatePath, "w")
    if idbw then idbw:write(os.date("%Y-%m-%d")); idbw:close() end
end
    ...
    lua_thread.create(function()
        ...
        while true do
            ...
            if status == "completed" or status == "canceled" then
                pcall(function() asyncReq:get() end)
                return
            elseif os.time() - startTime > 15 then
                pcall(function() asyncReq:cancel() end)
                return
            end
            wait(0)
...
function runMainLoop()
    ...
    while true do
        ...
        if os.time() - lastReportsCheck >= 5 then
...
function processTgQueue()
    ...
    local asyncReq = effil.thread(function(reqUrl, reqHeaders)
        local req = require("requests")
        local ok, result = pcall(req.get, reqUrl, { headers = reqHeaders, timeout = 10 })
        if ok and result then
            return result.text
        else
            return nil
                ...
                if success and doesFileExist(zipPath) then
                    ...
                    lua_thread.create(function()
                        ...
                        if extractOk and doesDirectoryExist(dirml.."/ResHelper/files/server_icons/") then
                            pcall(function()
                                os.execute('powershell -NoProfile -WindowStyle Hidden -NonInteractive -Command "Get-ChildItem -Path \\"' .. dirml .. '\\" -Recurse | Unblock-File"')
                            end)

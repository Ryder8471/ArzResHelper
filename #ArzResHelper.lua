...
-- Ни здесь, ни в loadGlobalPrices больше нет lua_thread.create — вместо этого
-- runMainLoop() (уже безопасно и постоянно работающий поток) сам проверяет
-- pricesRetryPending/pricesRequestActive на каждом тике. Это устраняет
-- вложенное создание потоков изнутри колбэка загрузки, которое роняло игру.
...
function loadGlobalPrices()
    ...
    downloadUrlToFile(
        ...
        tempFile,
        function(id, status)
            -- игнорируем колбэк от уже устаревшего запроса (страховочный
            -- таймаут в runMainLoop уже мог пойти дальше сам)
...
function hook.onServerMessage(color, text)
    local ok, err = pcall(function()
        ...
        -- AZ из VIP/рассрочки (приходит ДО чека) — копим в буфер с меткой времени,
        -- реальное начисление в paydayAZ произойдёт только при старте чека,
        -- и только для того, что попало в узкое окно перед ним (см. "БАНКОВСКИЙ ЧЕК")
        ...
        if text:find("БАНКОВСКИЙ ЧЕК") then
            ...
            -- Берём из буфера только AZ, пришедший за последние 20 секунд —
            -- всё, что старше (кейсы/рулетки в произвольный момент сессии), отбрасываем
            local now = os.time()
            local windowAZ = 0
        ...
        if cleanText:match("^%[Хранилище предметов%] Добавлен новый предмет 'Осколок Тайника") then
        ...
        if cleanText:match("^%[Хранилище предметов%] Добавлен новый предмет 'Осколок NFT контейнера") then
...
function drawItemMarketTab()
    ...
    if #allDates > 0 then
        ...
        if selectedDate and logCount > 0 then
            ...
            drawList:AddRectFilled(imgui.ImVec2(hdrX, hdrY), imgui.ImVec2(hdrX + listW, hdrY + hdrH), tableHdrBg, 4)
            drawList:AddText(imgui.ImVec2(hdrX + 12, hdrY + 3), tableHdrText, u8("Время"))
            ...
            if imTabIdx.v == 0 then
                ...
                drawList:AddText(imgui.ImVec2(col2, hdrY + 3), tableHdrText, u8("Зарплата"))
                drawList:AddText(imgui.ImVec2(col3, hdrY + 3), tableHdrText, u8("Депозит"))
                ...
                drawList:AddText(imgui.ImVec2(hdrX + listW - totalHeaderW - 12, hdrY + 3), iconCol, totalHeader)
            elseif imTabIdx.v == 1 then
                local col2 = hdrX + listW * 0.30
                local col3 = hdrX + listW * 0.62
                drawList:AddText(imgui.ImVec2(col2, hdrY + 3), tableHdrText, u8("Ник"))
                drawList:AddText(imgui.ImVec2(col3, hdrY + 3), tableHdrText, u8("Тип"))
                local sumHdr = u8("Стоимость"); local sumHdrW = imgui.CalcTextSize(sumHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW - sumHdrW - 12, hdrY + 3), tableHdrText, sumHdr)
            else
                local nameHdr = u8("Название"); local nameHdrW = imgui.CalcTextSize(nameHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW / 2 - nameHdrW / 2, hdrY + 3), tableHdrText, nameHdr)
                local sumHdr = u8("Стоимость"); local sumHdrW = imgui.CalcTextSize(sumHdr).x
                drawList:AddText(imgui.ImVec2(hdrX + listW - sumHdrW - 12, hdrY + 3), tableHdrText, sumHdr)
            end
            ...
            for _, log in ipairs(logs) do
                ...
                local rowBgCol = getCardColors(false)
                drawList:AddRectFilled(cardPos, imgui.ImVec2(cardPos.x + listW, cardPos.y + listH), rowBgCol, 6)
                drawList:AddRect(cardPos, imgui.ImVec2(cardPos.x + listW, cardPos.y + listH), borderCol, 6, 15, 1.0)
                ...
                if imTabIdx.v == 0 then
                    ...
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW - totalW - 12, cardPos.y + 10), iconCol, totalStr)
                elseif imTabIdx.v == 1 then
                    local col2 = cardPos.x + listW * 0.30
                    local col3 = cardPos.x + listW * 0.62
                    drawList:AddText(imgui.ImVec2(col2, cardPos.y + 10), 0xFFFFFFFF, log.nick or "")
                    ...
                    drawList:AddText(imgui.ImVec2(col3, cardPos.y + 10), typeColor, typeText)
                    local amtStr = formatNumber(log.amount) .. "$"; local amtW = imgui.CalcTextSize(amtStr).x
                    drawList:AddText(imgui.ImVec2(cardPos.x + listW - amtW - 12, cardPos.y + 10), iconCol, amtStr)
                else
...
function drawMainMenu()
    ...
    local function getColorHex(colorValue)
        if useCustomTheme then
    ...
    local function getColorVec4(colorValue)
        if useCustomTheme then
            return colorValue
        else
            if type(colorValue) == "number" then
...
function drawHomeTab()
    ...
    imgui.Spacing()
    imgui.TextColored(useCustomTheme and CUSTOM_THEME.cardTitle or imgui.ImVec4(0.26, 0.98, 0.26, 1.0), u8("Возможности:"))
    imgui.Spacing()
    ...
    for i, feat in ipairs(features) do
        ...
        local fH = 30
        bgCol, borderCol, iconCol, titleCol = getCardColors()
        drawList:AddRectFilled(imgui.ImVec2(fX, fY), imgui.ImVec2(fX + cardWidth - 4, fY + fH), bgCol, 6)
        drawList:AddText(imgui.ImVec2(fX + 12, fY + 7), iconCol, feat.icon)
...
function drawFarmTab()
    ...
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), borderCol, 6, 15, 1.0)
    drawList:AddText(imgui.ImVec2(cardX + 10, cardY + (cardH - 14) / 2), iconCol, fa.ICON_MONEY)
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + (cardH - 14) / 2), titleCol, u8("Цены за единицу"))
    imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
    ...
    drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), bgCol, 6)
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), borderCol, 6, 15, 1.0)
    drawList:AddText(imgui.ImVec2(cardX + 10, cardY + (cardH - 14) / 2), iconCol, fa.ICON_LINE_CHART)
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + (cardH - 14) / 2), titleCol, u8("Статистика"))
    imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
    ...
    drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), bgCol, 6)
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), borderCol, 6, 15, 1.0)
    drawList:AddText(imgui.ImVec2(cardX + 10, cardY + (cardH - 14) / 2), iconCol, fa.ICON_TELEVISION)
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + (cardH - 14) / 2), titleCol, u8("Оверлей"))
    
    local toggleX = cardX + listW - 45
    local toggleW = 36
    local toggleH = 20
    local toggleY = cardY + (cardH - toggleH) / 2
    local circleR = 8
    
    local toggleBgColor = settings.farmOverlayEnabled and 0xFF1AE591 or 0xFF444444
    drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), toggleBgColor, 10)
...
function drawMineTab()
    ...
    local toggleBgColor = settings.mineOverlayEnabled and 0xFF1AE591 or 0xFF444444
    drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), toggleBgColor, 10)
    ...
    drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), bgCol, 6)
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + listW, cardY + cardH), borderCol, 6, 15, 1.0)
    drawList:AddText(imgui.ImVec2(cardX + 10, cardY + (cardH - 14) / 2), iconCol, fa.ICON_CLOCK_O)
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + (cardH - 14) / 2), titleCol, u8("Таймер спавна"))
    ...
    local toggleY = cardY + (cardH - toggleH) / 2
    local circleR = 8
    local toggleBgColor2 = settings.mineSpawnTimerEnabled and 0xFF1AE591 or 0xFF444444
    drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), toggleBgColor2, 10)
...
function drawSawmillTab()
    ...
    local toggleBgColor = settings.sawmillOverlayEnabled and 0xFF1AE591 or 0xFF444444
    drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), toggleBgColor, 10)
...
function drawLeaderboardTab()
    ...
    if not getLbEnabled() then
        ...
        if showLoading and not (cache and #cache > 0) then
            -- Пробуем загрузить из оффлайн-кэша
            local mode = modesList[lbModeTab.v + 1]
            local period = periodsList[lbTab.v + 1]
            local cacheFile = io.open(dirml .. "/lb_cache_" .. mode .. "_" .. period .. "_cache.json", "r")
            if cacheFile then
                local content = cacheFile:read("*all")
                cacheFile:close()
                local decoded = u8:decode(content)
        ...
        elseif cache and #cache > 0 then
            ...
            local isMe = (myName == developerName)
            local devEntry = nil
            local normalCache = {}
            ...
            for _, entry in ipairs(cache) do
                if entry.name == developerName then
                    ...
                    table.insert(normalCache, entry)
                end
            end
            ...
            local col1W, col3W, col4W, col5W, col2W
            if hasResources then
                col1W = 35; col3W = serverColW; col4W = 120; col5W = 110
                col2W = tableWidth - (col1W + col3W + col4W + col5W)
            else
                col1W = 35; col3W = serverColW; col4W = 130
                col5W = (isIncome or isIM) and 120 or 0
                col2W = tableWidth - (col1W + col3W + col4W + col5W)
            end
            
            -- Карточка разработчика 
            if devEntry and isMe then
                local devW = tableWidth
                local colX = hasResources and {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W}
                    or (isIncome or isIM) and {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W}
                    or {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W}
                
                local colsToDivide = hasResources and {colX[2], colX[3], colX[4], colX[5]} or (isIncome or isIM) and {colX[2], colX[3], colX[4], colX[5]} or {colX[2], colX[3], colX[4]}
                
                -- Заголовок
                drawList:AddRectFilled(imgui.ImVec2(pos.x, pos.y), imgui.ImVec2(pos.x + devW, pos.y + headerHeight + rowHeight), 0xFF141414, 6)
                drawList:AddRect(imgui.ImVec2(pos.x, pos.y), imgui.ImVec2(pos.x + devW, pos.y + headerHeight + rowHeight), 0xFF2A2A2A, 6, 15, 1.5)
                
                drawList:AddText(imgui.ImVec2(colX[1] + 10, pos.y + 7), 0xFF4444FF, u8("#"))
                drawList:AddText(imgui.ImVec2(colX[2] + 10, pos.y + 7), 0xFF4444FF, u8("Разработчик"))
                drawList:AddText(imgui.ImVec2(colX[3] + 10, pos.y + 7), 0xFF4444FF, u8("Сервер"))
                drawList:AddText(imgui.ImVec2(colX[4] + 10, pos.y + 7), 0xFF4444FF, u8("Доход"))
                if hasResources then
                    drawList:AddText(imgui.ImVec2(colX[5] + 10, pos.y + 7), 0xFF4444FF, u8("Ресурсы"))
                elseif isIncome or isIM then
                    drawList:AddText(imgui.ImVec2(colX[5] + 10, pos.y + 7), 0xFF4444FF, u8("Подробнее"))
                end
                
                drawList:AddLine(imgui.ImVec2(pos.x + 6, pos.y + headerHeight), imgui.ImVec2(pos.x + devW - 6, pos.y + headerHeight), 0xFF2A2A2A, 1.5)
                for _, cx in ipairs(colsToDivide) do
                    drawList:AddLine(imgui.ImVec2(cx, pos.y + 8), imgui.ImVec2(cx, pos.y + headerHeight + rowHeight - 4), 0xFF2A2A2A, 1.0)
                end
                
                -- Строка разработчика
                local devRowY = pos.y + headerHeight
                for _, cx in ipairs(colsToDivide) do
                    drawList:AddLine(imgui.ImVec2(cx, devRowY + 2), imgui.ImVec2(cx, devRowY + rowHeight - 2), 0xFF252525, 0.5)
                end
                
                drawList:AddText(imgui.ImVec2(colX[1] + 10, devRowY + 7), 0xFF4444FF, fa.ICON_WRENCH)
                drawList:AddText(imgui.ImVec2(colX[2] + 10, devRowY + 7), 0xFFFFFFFF, devEntry.name)
                if devEntry.server and serverIcons[devEntry.server] then
                    drawList:AddImage(serverIcons[devEntry.server], imgui.ImVec2(colX[3] + 10, devRowY + 6), imgui.ImVec2(colX[3] + 26, devRowY + 22))
                    drawList:AddText(imgui.ImVec2(colX[3] + 30, devRowY + 7), 0xFF66CCFF, devEntry.server)
                else
                    drawList:AddText(imgui.ImVec2(colX[3] + 10, devRowY + 7), 0xFF66CCFF, devEntry.server or "")
                end
                drawList:AddText(imgui.ImVec2(colX[4] + 10, devRowY + 7), 0xFF33CC33, formatNumber(devEntry.amount) .. "$")
                
                -- Кнопка Подробнее или Ресурсы
                if hasResources then
                    if devEntry.resources then
                        local resKeys = {}
                        for k, _ in pairs(devEntry.resources) do resKeys[#resKeys + 1] = k end
                        table.sort(resKeys)
                        local btnX = colX[5] + 10; local btnY = devRowY + 5
                        ...
                        imgui.SetCursorScreenPos(imgui.ImVec2(btnX, btnY))
                        imgui.InvisibleButton("##dev_res", imgui.ImVec2(90, 22))
                        if imgui.IsItemHovered() then
                            imgui.BeginTooltip()
                ...
                elseif isIncome or isIM then
                    ...
                    imgui.SetCursorScreenPos(imgui.ImVec2(btnX, btnY))
                    if imgui.InvisibleButton("##dev_detail", imgui.ImVec2(100, 22)) then
                ...
                pos.y = pos.y + headerHeight + rowHeight + 3
                imgui.SetCursorScreenPos(imgui.ImVec2(pos.x, pos.y))
                imgui.Dummy(imgui.ImVec2(tableWidth, 3))
                pos.y = pos.y + 3
                drawList:AddLine(imgui.ImVec2(pos.x, pos.y), imgui.ImVec2(pos.x + tableWidth, pos.y), 0xFF333333, 1.0)
                imgui.SetCursorScreenPos(imgui.ImVec2(pos.x, pos.y + 1))
                imgui.Dummy(imgui.ImVec2(tableWidth, 5))
                pos.y = pos.y + 7
            end
            
            -- Таблица обычных игроков (без изменений)
            if not lbServerFilter then lbServerFilter = imgui.ImInt(0) end
            local serverList = {u8("Все сервера (0)")}
            local serverListClean = {u8("Все сервера")}
            local serverSet = {}
            local serverCounts = {}
            local totalPlayers = #normalCache
            for _, entry in ipairs(normalCache) do
                if entry.server then
                    ...
                    if not serverSet[entry.server] then
                        ...
                        table.insert(serverListClean, entry.server)
                    end
                end
            end
            serverList[1] = u8("Все сервера") .. " (" .. totalPlayers .. ")"
            ...
            table.sort(serverListClean, function(a, b)
            ...
            serverList = {}
            for _, cleanName in ipairs(serverListClean) do
                if cleanName == u8("Все сервера") then
                    table.insert(serverList, u8("Все сервера") .. " (" .. totalPlayers .. ")")
                else
                    local count = serverCounts[cleanName] or 0
                    table.insert(serverList, cleanName .. " (" .. count .. ")")
                end
            end
            
            local filteredCache = normalCache
            if lbServerFilter.v > 0 and lbServerFilter.v <= #serverListClean then
                local selectedServer = serverListClean[lbServerFilter.v + 1]
                ...
                if imgui.IsItemHovered() then
                    ...
                    if lbServerFilter.v > 0 then
                        ...
                        if activeServer then
                            imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1), u8("Активен: ") .. activeServer)
                        end
                    end
                    imgui.EndTooltip()
                end

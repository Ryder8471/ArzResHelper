...
function imgui.OnDrawFrame()
    ...
    if mainWin.v then
        ...
        local btnHeight = 38
        local spacing = 4  -- фиксированный отступ между кнопками
        ...
        for idx, item in ipairs(menuItems) do
            ...
            if isActive then
                drawList:AddRect(imgui.ImVec2(btnPosX, btnPosY), imgui.ImVec2(btnPosX + 178, btnPosY + btnHeight), theme.borderActive, 5, 15, 1.5)
            end
        ...
        elseif select_menu[5] then
            ...
            if #bindDatabase.binds == 0 then
                ...
                local bg = theme.rightPanelBg
                local b = math.floor((bg % 0x1000000) / 0x10000) / 255
                local g = math.floor((bg % 0x10000) / 0x100) / 255
                local r = math.floor(bg % 0x100) / 255

                for key, val in ipairs(bindDatabase.binds) do
                    -- Отступ слева 5 пикселей
                    imgui.SetCursorPosX(imgui.GetCursorPosX() + 5)
                    
                    -- Запоминаем позицию для обводки
                    local bindPos = imgui.GetCursorScreenPos()
                    local bindWidth = imgui.GetWindowWidth() - 30
                    local bindHeight = 36
                    
                    imgui.PushStyleColor(imgui.Col.ChildWindowBg, imgui.ImVec4(r, g, b, 1.0))
                    imgui.BeginChild("bind_row_" .. key, imgui.ImVec2(bindWidth, bindHeight), false)
                    ...
                    -- Яркая акцентная обводка ПОСЛЕ плашки
                    local drawList = imgui.GetWindowDrawList()
                    drawList:AddRect(
                        imgui.ImVec2(bindPos.x - 1, bindPos.y - 1),
                        imgui.ImVec2(bindPos.x + bindWidth + 1, bindPos.y + bindHeight + 1),
                        theme.borderActive, 4, 15, 2.0
                    )

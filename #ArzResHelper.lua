...
function hook.onServerMessage(color, text)
    ...
    if text:match("^%[Item Market%]") then
        ...
        if afterNachisleno then
            local numbers = {}
            for n in afterNachisleno:gmatch("%d+") do
                table.insert(numbers, tonumber(n))
            end
            if #numbers == 1 then
                -- Только одно число (например 9000) — это может быть тысячи или полная сумма
                -- Если есть :k: — это тысячи, умножаем на 1000
                if afterNachisleno:find(":k:") then
                    amount = numbers[1] * 1000
                else
                    amount = numbers[1]
                end
            elseif #numbers >= 2 then
                amount = numbers[1] * 1000000
                for i = 2, #numbers do
                    amount = amount + numbers[i] * (1000 ^ (#numbers - i))
                end
            end
        end
...
function drawItemMarketTab()
    ...
    local weekDates = {}
    for i = 0, currentDay - 1 do
        weekDates[getGameDate(os.time() - i * 86400)] = true
    end
    ...
    for _, log in ipairs(itemMarketLog) do
        ...
        if weekDates[ld] then weekIM = weekIM + log.amount end
    end
    ...
    for _, dateStr in ipairs(datesOrder) do
        ...
        drawList:AddRectFilled(imgui.ImVec2(dateCardX, dateCardY), imgui.ImVec2(dateCardX + listW, dateCardY + dateCardH), 
            hovered and 0xFF1A2E1A or 0xFF141414, 6)
        ...
        if datesExpanded[dateStr] == nil then datesExpanded[dateStr] = true end
        ...
        if datesExpanded[dateStr] then
            ...
            drawList:AddRectFilled(imgui.ImVec2(colCardX, colCardY), imgui.ImVec2(colCardX + listW, colCardY + colCardH), 0xFF1A1A1A, 4)
            drawList:AddText(imgui.ImVec2(colCardX + 12, colCardY + 3), 0xFF888888, u8("Время"))
            local nickHdr = u8("Ник")
            local nickHdrW = imgui.CalcTextSize(nickHdr).x
            drawList:AddText(imgui.ImVec2(colCardX + listW / 2 - nickHdrW / 2, colCardY + 3), 0xFF888888, nickHdr)
            local sumHdr = u8("Заработано")
            local sumHdrW = imgui.CalcTextSize(sumHdr).x
            ...
            for _, log in ipairs(logs) do
                ...
                drawList:AddRect(cardPos, imgui.ImVec2(cardPos.x + listW, cardPos.y + listH), 0xFF2A2A2A, 6, 15, 1.0)
                ...
                local amtStr = formatNumber(log.amount) .. "$"
                local amtW = imgui.CalcTextSize(amtStr).x

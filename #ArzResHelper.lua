...
local function getGameDate(timestamp)
    ...
    -- Получаем Y-m-d через деление (не зависит от часового пояса системы)
    local days = math.floor(msk / 86400)
    -- Переводим дни в дату (формула для UNIX-времени)
    local y = 1970
...
local function getMaxStack(resKey)
    local maxStacks = {
        flax = 2998, cotton = 2998, rare_tkan = 5000, water = 1000, dye = 100, coal = 5000,
        stone = 2998, metal = 2998, bronze = 2998, silver = 2998, gold = 2998,
...
if btnHovered and imgui.IsMouseClicked(0) then
    if scanState.active then
        -- Ничего не делаем, идёт сканирование
    else

...
if not host or host == "" then
    host = sampGetServerAddress()
end

if host and host:find(":") then
    host = host:match("([^:]+)")
end
...
function drawItemMarketTab()
    ...
    for _, log in ipairs(itemMarketLog) do
        local d = getGameDate(log.time)
        if not seenDates[d] then
            seenDates[d] = true
            table.insert(allDates, d)
        end
    end
table.sort(allDates, function(a, b)
    if not a or not b then return false end
    local ya, ma, da = a:match("(%d+)-(%d+)-(%d+)")
    local yb, mb, db = b:match("(%d+)-(%d+)-(%d+)")
    ...
    if #allDates > 0 then
        ...
        -- Считаем сумму за выбранный день ДО отрисовки карточки
        local selectedDate = allDates[imSelectedDate.v + 1]
        local daySum = 0
        local logCount = 0
        if selectedDate then
            for i = 1, math.min(#itemMarketLog, 500) do
                local log = itemMarketLog[i]
                local d = getGameDate(log.time)
                if d == selectedDate then
                    daySum = daySum + log.amount
                    logCount = logCount + 1
                end
            end
        end
        ...
        drawList:AddRectFilled(btnPos, imgui.ImVec2(btnPos.x + btnW, btnPos.y + btnH), 
            hovered and 0xFF222222 or 0xFF1A1A1A, 6)
        drawList:AddRect(btnPos, imgui.ImVec2(btnPos.x + btnW, btnPos.y + btnH), 
            hovered and 0xFF555555 or 0xFF333333, 6, 15, 1.0)
        
        -- Дата слева
        drawList:AddText(imgui.ImVec2(btnPos.x + 12, btnPos.y + 10), 0xFF1AE591, fa.ICON_CALENDAR .. " " .. selectedDate)
        
        -- Сумма за день справа в этой же карточке
        local sumText = formatNumber(daySum) .. "$"
local infoText = "  •  " .. logCount .. u8(" аренд")
...
drawList:AddText(imgui.ImVec2(btnPos.x + btnW - infoTextW - 35, btnPos.y + 10), 0xFF888888, infoText)
    ...
    for i, d in ipairs(allDates) do
        if imgui.Selectable(d, i == imSelectedDate.v + 1) then
            imSelectedDate.v = i - 1
    ...
    imgui.EndPopup()
        ...
        -- Список аренд за выбранную дату
...
if btnHovered and imgui.IsMouseClicked(0) then
    if scanState.active then
        ...
        if select_menu[1] then
            ...
            imgui.Separator()
            imgui.Spacing()
            if workTypeSelected then
                imgui.TextColored(imgui.ImVec4(0.26, 0.98, 0.26, 1.0), u8("Текущий режим: " .. config.name))
            else
                imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.2, 1), u8("Режим не выбран. Выберите Ферму, Шахту или Лесопилку в меню слева."))
            end
                ...
                if StyleButton(u8("Показать рейтинг"), fa.ICON_SEARCH) then
                    ...
                    if mode == "Income" then
                        lua_thread.create(function()
                            ...
                            local modeNames = {Farm = "Ферма", Mine = "Шахта", Sawmill = "Лесопилка", IM = "Item Market"}
                ...
                elseif cache and #cache > 0 then
                    ...
                    if hasResources and cache[1].resources then
                        ...
                        local colX = {
                            ...
                            pos.x + col1W, 
                            pos.x + col1W + col2W, 
                            pos.x + col1W + col2W + col3W, 
                            pos.x + col1W + col2W + col3W + col4W, 
                            pos.x + col1W + col2W + col3W + col4W + col5W
                        }
                        ...
                        for i, entry in ipairs(cache) do
                            ...
                            local btnY = y + 5
                            local btnHovered = (imgui.GetMousePos().x >= btnX and imgui.GetMousePos().x <= btnX + 90 and 
                                               imgui.GetMousePos().y >= btnY and imgui.GetMousePos().y <= btnY + 22)
                            
                            drawList:AddRectFilled(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 90, btnY + 22), 
                            ...
                            drawList:AddText(imgui.ImVec2(btnX + (90 - detailW) / 2, btnY + 3), 
                        ...
                        local col3W = serverColW
                        local col4W = 120
                        local col5W = isIncome and 120 or 0
                        local col2W = tableWidth - (col1W + col3W + col4W + col5W)
                        
                        local colX = isIncome and {
                            pos.x, pos.x + col1W, pos.x + col1W + col2W, 
                            pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W
                        } or {
                            pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W
                        }
                        ...
                        for i, entry in ipairs(cache) do
                            ...
                            if isIncome then
                                local btnX = colX[5] + 10
                                local btnY = y + 5
                                local btnHovered = (imgui.GetMousePos().x >= btnX and imgui.GetMousePos().x <= btnX + 100 and 
                                                   imgui.GetMousePos().y >= btnY and imgui.GetMousePos().y <= btnY + 22)
                                
                                drawList:AddRectFilled(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 100, btnY + 22), 
                                    btnHovered and 0xFF333333 or 0xFF252525, 4)
                                drawList:AddRect(imgui.ImVec2(btnX, btnY), imgui.ImVec2(btnX + 100, btnY + 22), 0xFF3A3A3A, 4, 15, 1.0)
                                
                                local detailText = u8("Подробнее")
                                local detailW = imgui.CalcTextSize(detailText).x
                                drawList:AddText(imgui.ImVec2(btnX + (100 - detailW) / 2, btnY + 3), 
                                    btnHovered and 0xFFFFFFFF or 0xFF1AE591, detailText)
...
function checkAndInstallResources()
    local firstRunFlag = dirml .. "/.reshelper_installed"
    if doesFileExist(firstRunFlag) then return end
    
    local needInstall = false
    if not doesFileExist(dirml .. "/ResHelper/files/logo-ArzResHelper.png") then needInstall = true end
    if not doesFileExist(dirml .. "/resource/farm/pickup.wav") then needInstall = true end
    
    if not needInstall then
        local f = io.open(firstRunFlag, "w")
        if f then f:write("1"); f:close() end
        return
    end
    
    sampAddChatMessage(SCRIPT_PREFIX .. "Не найдены файлы ресурсов. Скачиваю...", SCRIPT_COLOR)
    ...
    downloadUrlToFile(
        ...
        function(id, status)
            ...
            if status == dlstatus.STATUS_ENDDOWNLOADDATA or doesFileExist(zipPath) then
                installed = true
                sampAddChatMessage(SCRIPT_PREFIX .. "Архив скачан. Устанавливаю ресурсы...", SCRIPT_COLOR)
                
                local vbsPath = dirml .. "\\_extract.vbs"
                local vbs = io.open(vbsPath, "w")
                if vbs then
                    vbs:write('Set objShell = CreateObject("Shell.Application")\n')
                    vbs:write('Set objFSO = CreateObject("Scripting.FileSystemObject")\n')
                    vbs:write('zipFile = "' .. zipPath .. '"\n')
                    vbs:write('destFolder = "' .. dirml .. '"\n')
                    vbs:write('If Not objFSO.FolderExists(destFolder) Then objFSO.CreateFolder(destFolder)\n')
                    vbs:write('Set zip = objShell.NameSpace(zipFile)\n')
                    vbs:write('Set dest = objShell.NameSpace(destFolder)\n')
                    vbs:write('dest.CopyHere zip.Items(), 16\n')
                    vbs:write('While objFSO.FileExists(zipFile) = False\n')
                    vbs:write('  WScript.Sleep 100\n')
                    vbs:write('Wend\n')
                    vbs:close()
                end
                os.execute('wscript.exe "' .. vbsPath .. '"')
                
                lua_thread.create(function()
                    wait(5000)
                    if doesFileExist(vbsPath) then os.remove(vbsPath) end
                    
                    if not doesFileExist(dirml .. "/ResHelper/files/logo-ArzResHelper.png") and doesFileExist(zipPath) then
                        local psCmd = 'powershell -WindowStyle Hidden -NoProfile -ExecutionPolicy Bypass -command "Expand-Archive -LiteralPath \\"' .. zipPath .. '\\" -DestinationPath \\"' .. dirml .. '\\" -Force"'
                        os.execute(psCmd)
                        wait(3000)
                    end
                    ...
                    if not doesFileExist(dirml .. "/ResHelper/files/logo-ArzResHelper.png") then
                        sampAddChatMessage(SCRIPT_PREFIX .. "Ошибка установки. Скачайте архив с GitHub вручную и распакуйте в папку moonloader.", SCRIPT_COLOR)
                    end
                    
                    local f = io.open(firstRunFlag, "w")
                    if f then f:write("1"); f:close() end
                    sampAddChatMessage(SCRIPT_PREFIX .. "Ресурсы установлены! Перезагрузите скрипт: /rhrl", SCRIPT_COLOR)
                end)
            elseif status == dlstatus.STATUSEX_ENDDOWNLOAD and not doesFileExist(zipPath) then
                installed = true
                sampAddChatMessage(SCRIPT_PREFIX .. "Ошибка загрузки ресурсов. Проверьте интернет.", SCRIPT_COLOR)
            end
        end
    )
end

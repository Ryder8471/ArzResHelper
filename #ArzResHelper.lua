...
function migrateStatsBetweenModes(workType, fromCustom, toCustom)
    ...
    -- Читаем существующие данные в целевом файле
    local existingLogs = {}
    ...
    -- Читаем данные из исходного файла и пересчитываем value
    ...
    -- Если это текущий тип работы, перезагружаем логи
...
if useCustomFarmPrices or useCustomMinePrices or useCustomSawmillPrices then return end
    ...
    -- Income больше не отправляется — пересчитывается на сервере Google
...
local function queueTelegramMessage(text)
...
function hook.onShowDialog(id, style, title, button1, button2, text)
    ...
    if not ok then
        ...
        -- В случае ошибки при сканировании, сбрасываем состояние чтобы не заблокировать интерфейс
        if scanState.active then
            scanState.active = false
            scanState.scanning = false
            scanState.waitForInventory = false
...
function drawSettingsTab()
    ...
    if settingsExpandedLeaderboard then
        ...
        if useCustomFarmPrices or useCustomMinePrices or useCustomSawmillPrices then
            -- Блокируем переключатель если используются свои цены
            local drawList = imgui.GetWindowDrawList()
            local pos = imgui.GetCursorScreenPos()
            local switchWidth = 36
            local switchHeight = 20
    ...
    drawList:AddText(imgui.ImVec2(cardX + 35, cardY + 10), 0xFF1AE591, u8("Настройки дохода"))
    imgui.SetCursorScreenPos(imgui.ImVec2(cardX, cardY))
...
function drawMineOverlay()
    ...
    imgui.End(); imgui.PopStyleColor(5); imgui.PopStyleVar(2)
end
...
function drawMainMenu()
    ...
    drawList:AddRectFilled(imgui.ImVec2(winPos.x, winPos.y), imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y + winSize.y), imVec4ToHex(theme.leftPanelBg), 6, 9)
    drawList:AddLine(imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y), imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y + winSize.y), imVec4ToHex(theme.borderColor), 1.0)
    drawList:AddRectFilled(imgui.ImVec2(winPos.x + 6, winPos.y), imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y + 45), imVec4ToHex(theme.titleBg), 0, 0)
    drawList:AddRectFilled(imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y), imgui.ImVec2(winPos.x + winSize.x - 6, winPos.y + 45), imVec4ToHex(theme.rightTitleBg), 0, 6)
    drawList:AddLine(imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y + 8), imgui.ImVec2(winPos.x + leftPanelWidth, winPos.y + 37), imVec4ToHex(theme.borderColor), 1.0)
    ...
    if useCustomTheme then imgui.TextColored(theme.accent, fa.ICON_WRENCH .. "  Resource Helper")
    else imgui.TextColored(hexToImVec4(theme.accent), fa.ICON_WRENCH .. "  Resource Helper") end
    ...
    if useCustomTheme then
        drawList:AddText(imgui.ImVec2(winPos.x + leftPanelWidth + 20, winPos.y + 15), theme.headerTitle, headerText)
    else
        drawList:AddText(imgui.ImVec2(winPos.x + leftPanelWidth + 20, winPos.y + 15), imVec4ToHex(hexToImVec4(theme.headerTitle)), headerText)
    end
...
function drawMenuButtons(winPos, winSize, leftPanelWidth, theme)
    ...
    for idx, item in ipairs(menuItems) do
        if item.header then
            ...
            if isActive then drawList:AddRectFilled(imgui.ImVec2(btnPosX, btnPosY), imgui.ImVec2(btnPosX + 178, btnPosY + btnHeight), imVec4ToHex(theme.buttonActive), 5)
            elseif btnHovered then drawList:AddRectFilled(imgui.ImVec2(btnPosX, btnPosY), imgui.ImVec2(btnPosX + 178, btnPosY + btnHeight), imVec4ToHex(theme.buttonHover), 5) end
            if isActive then drawList:AddRect(imgui.ImVec2(btnPosX, btnPosY), imgui.ImVec2(btnPosX + 178, btnPosY + btnHeight), imVec4ToHex(theme.borderActive), 5, 15, 1.5) end
...
function drawRightPanelContent(winPos, leftPanelWidth, winSize, theme)
    ...
    elseif currentTab == 11 then drawAboutTab() end
...
function drawHomeTab()
    ...
    local cardH = editingMenuKey and 54 or 38
    
    drawList:AddRectFilled(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + cardW, cardY + cardH), 0xFF1A1A1A, 8)
    drawList:AddRect(imgui.ImVec2(cardX, cardY), imgui.ImVec2(cardX + cardW, cardY + cardH), editingMenuKey and 0xFF1AE591 or 0xFF333333, 8, 15, 1.5)
    
    local contentCenterY = cardY + cardH / 2
    
    drawList:AddText(imgui.ImVec2(cardX + 15, contentCenterY - 8), 0xFF1AE591, fa.ICON_KEYBOARD_O)
...
function drawFarmTab()
    ...
    if pricesExpandedFarm then
        imgui.Spacing()
        -- Переключатель кастомных цен фермы
    ...
    if statsExpandedFarm then
        ...
        elseif farmStatsTab.v == 1 then
            ...
            for _, k in ipairs(config.resourceOrder) do
                local price = getPriceForResource(k, WORK_TYPES.FARM)
                weekTotal = weekTotal + ((weekData[k] or 0) * price)
            end
            imgui.Text(u8("Доход за неделю: ")); imgui.SameLine(); 
            imgui.TextColored(imgui.ImVec4(0.3, 1.0, 0.3, 1), formatNumber(weekTotal) .. "$")
        elseif farmStatsTab.v == 2 then
            ...
            if #availableDates > 0 then
                ...
                if idx <= #availableDates then
                    ...
                    for _, k in ipairs(config.resourceOrder) do
                        local price = getPriceForResource(k, WORK_TYPES.FARM)
                        dayTotal = dayTotal + ((dayData[k] or 0) * price)
                    end
                    imgui.Text(u8("Доход: ")); imgui.SameLine(); 
                    imgui.TextColored(imgui.ImVec4(0.3, 1.0, 0.3, 1), formatNumber(dayTotal) .. "$")
                end
...
function drawMineTab()
    ...
    if pricesExpandedMine then
        imgui.Spacing()
        -- Переключатель кастомных цен шахты
    ...
    toggleX = cardX + listW - 45
    toggleY = cardY + 6
    bgColor = settings.mineSpawnTimerEnabled and 0xFF1AE591 or 0xFF444444
    drawList:AddRectFilled(imgui.ImVec2(toggleX, toggleY), imgui.ImVec2(toggleX + toggleW, toggleY + toggleH), bgColor, 10)
    
    circleX = settings.mineSpawnTimerEnabled and (toggleX + toggleW - circleR - 3) or (toggleX + circleR + 3)
    drawList:AddCircleFilled(imgui.ImVec2(circleX, toggleY + toggleH / 2), circleR, 0xFFFFFFFF)
...
function drawSawmillTab()
    ...
    if pricesExpandedSaw then
        imgui.Spacing()
        -- Переключатель кастомных цен лесопилки
...
function drawLeaderboardTab()
    ...
    if not getLbEnabled() then
        ...
        elseif cache and #cache > 0 then
            ...
            -- Сбор списка серверов для фильтра
            if not lbServerFilter then lbServerFilter = imgui.ImInt(0) end
            local serverList = {u8("Все сервера")}
            local serverSet = {}
            for _, entry in ipairs(cache) do
                if entry.server and not serverSet[entry.server] then
                    serverSet[entry.server] = true
                    table.insert(serverList, entry.server)
                end
            end
            ...
            -- Фильтрация по выбранному серверу
            local filteredCache = cache
            if lbServerFilter.v > 0 and lbServerFilter.v <= #serverList then
                local selectedServer = serverList[lbServerFilter.v + 1]
                if selectedServer ~= u8("Все сервера") then
                    filteredCache = {}
                    for _, entry in ipairs(cache) do
                        if entry.server == selectedServer then
                            table.insert(filteredCache, entry)
                        end
                    end
                end
            end
            ...
            if hasResources then
                local resKeys = {}
                if filteredCache[1] and filteredCache[1].resources then
                    for k, _ in pairs(filteredCache[1].resources) do resKeys[#resKeys + 1] = k end
                    table.sort(resKeys)
                end
                
                local maxServerLen = 5
                for _, e in ipairs(filteredCache) do if e.server and #e.server > maxServerLen then maxServerLen = #e.server end end
                ...
                local colX = {pos.x, pos.x + col1W, pos.x + col1W + col2W, pos.x + col1W + col2W + col3W, pos.x + col1W + col2W + col3W + col4W}
                
                drawList:AddRectFilled(pos, imgui.ImVec2(pos.x + tableWidth, pos.y + headerHeight + #filteredCache * rowHeight), 0xFF141414, 6)
                drawList:AddRect(pos, imgui.ImVec2(pos.x + tableWidth, pos.y + headerHeight + #filteredCache * rowHeight), 0xFF2A2A2A, 6, 15, 1.5)
                ...
                -- Заголовок Сервер с фильтром
                local serverHeaderX = colX[3] + 10
                local serverHeaderY = pos.y + 7
                local serverHeaderText = u8("Сервер")
                local filterArrow = lbServerFilter.v > 0 and " " .. fa.ICON_FILTER or ""
                local serverHeaderFull = serverHeaderText .. filterArrow .. " " .. fa.ICON_ANGLE_DOWN
                local serverHeaderFullW = imgui.CalcTextSize(serverHeaderFull).x
                drawList:AddText(imgui.ImVec2(serverHeaderX, serverHeaderY), lbServerFilter.v > 0 and 0xFFFFAA00 or 0xFF1AE591, serverHeaderFull)
                
                -- Невидимая кнопка для клика по заголовку Сервер
                imgui.SetCursorScreenPos(imgui.ImVec2(colX[3], pos.y))
                if imgui.InvisibleButton("##server_filter_btn", imgui.ImVec2(col3W, headerHeight)) then
                    imgui.OpenPopup("##server_filter_popup")
                end
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.Text(u8("Нажмите для фильтрации по серверу"))
                    if lbServerFilter.v > 0 then
                        imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1), u8("Активен: ") .. serverList[lbServerFilter.v + 1])
                    end
                    imgui.EndTooltip()
                end
                
                if imgui.BeginPopup("##server_filter_popup") then
                    imgui.Text(u8("Фильтр по серверу:"))
                    imgui.Separator()
                    for i, srv in ipairs(serverList) do
                        if imgui.Selectable(srv, lbServerFilter.v == i - 1) then
                            lbServerFilter.v = i - 1
                            imgui.CloseCurrentPopup()
                        end
                    end
                    imgui.EndPopup()
                ...
                drawList:AddLine(imgui.ImVec2(pos.x + 6, pos.y + headerHeight), imgui.ImVec2(pos.x + tableWidth - 6, pos.y + headerHeight), 0xFF2A2A2A, 1.5)
                for _, cx in ipairs({colX[2], colX[3], colX[4], colX[5]}) do drawList:AddLine(imgui.ImVec2(cx, pos.y + 8), imgui.ImVec2(cx, pos.y + headerHeight + #filteredCache * rowHeight - 4), 0xFF2A2A2A, 1.0) end
                ...
                imgui.SetCursorScreenPos(imgui.ImVec2(pos.x, pos.y + headerHeight + #filteredCache * rowHeight + 8))
                imgui.Dummy(imgui.ImVec2(tableWidth, 0))
            else
                local maxServerLen = 5
                for _, e in ipairs(filteredCache) do if e.server and #e.server > maxServerLen then maxServerLen = #e.server end end
                ...
                -- Невидимая кнопка для клика по заголовку Сервер
                imgui.SetCursorScreenPos(imgui.ImVec2(colX[3], pos.y))
                if imgui.InvisibleButton("##server_filter_btn2", imgui.ImVec2(col3W, headerHeight)) then
                ...
                for _, cx in ipairs(colsToDivide) do drawList:AddLine(imgui.ImVec2(cx, pos.y + 8), imgui.ImVec2(cx, pos.y + headerHeight + #filteredCache * rowHeight - 4), 0xFF2A2A2A, 1.0) end
                        ...
                        if isIM then
                            ...
                            if imgui.IsItemHovered() then
                                ...
                                if entry.resources then
                                    imVal = entry.resources["im"] or 0
                                    shVal = entry.resources["shards"] or 0
                                end
...
function imgui.OnDrawFrame()
    ...
    if mainWin.v then drawMainMenu() end
